...
class MarketRegimeDetector:
    ...
    def detect(self, df):
        ...
        try:
            atr, atr_avg = self._calculate_atr(df)
            adx_series = self._calculate_adx(df)
            adx = adx_series.iloc[-1] if not adx_series.empty else 20
            # حساب ميل الـ ADX للكشف عن تباطؤ الزخم (هل الترند يتسارع أم ينتهي؟)
            adx_slope = (adx - adx_series.iloc[-3]) / 3 if len(adx_series) >= 3 else 0
            
            trend_strength = self._get_trend_strength(df)
            
            regime = self._classify_regime(adx, adx_slope, atr, atr_avg, trend_strength)
    ...
    def _calculate_atr(self, df):
        ...
        try:
            ...
            atr_avg_val = atr_avg.iloc[-1] if not atr_avg.empty else atr_val
            
            return atr_val, atr_avg_val
        except Exception:
            return 0, 0
...
class FlashCrashDetector:
    ...
    def __init__(self):
        self.flash_crash_threshold = 5.0  # خفض العتبة لسرعة الاستجابة
        self.whale_dump_threshold = 3.0   # خفض العتبة لكشف الحيتان مبكراً
    ...
    def _detect_flash_crash(self, df):
        ...
        try:
            ...
            open_5min_ago = df['open'].iloc[-5] if len(df) >= 5 else df['open'].iloc[0]
...
class TimeAnalyzer:
    ...
    def analyze(self):
        ...
        return {
            ...
            'is_asian_session':      self._is_in_session('asian', hour),
            'is_european_session':   self._is_in_session('european', hour),
            'is_american_session':   self._is_in_session('american', hour),
            'current_session':       self._get_current_session(hour),
            'volatility_expectation': self._get_volatility_expectation(hour),
            'trading_recommendation': self._get_recommendation(hour),
        }
    
    def _is_in_session(self, session, hour):
    ...
    def _get_current_session(self, hour):
        """أي جلسة نشطة الآن؟"""
        if self._is_in_session('american', hour):
            return 'AMERICAN'
        elif self._is_in_session('european', hour):
            return 'EUROPEAN'
        elif self._is_in_session('asian', hour):
            return 'ASIAN'
        return 'QUIET'
    
    def _get_volatility_expectation(self, hour):
        """توقعات التقلبات"""
    ...
    def _get_recommendation(self, hour):
...
# ================================================================
# 🎯 QUICK FUNCTIONS - Singleton instances (fix OOM)
# ================================================================

_regime_detector = MarketRegimeDetector()
_crash_detector  = FlashCrashDetector()
_time_analyzer   = TimeAnalyzer()

def get_market_regime(df):
    """كشف حالة السوق"""
    return _regime_detector.detect(df)


def check_flash_crash(df, symbol=None):
    """كشف Flash Crash"""
    return _crash_detector.detect(df, symbol)


def get_time_analysis():
    """تحليل الوقت"""
    return _time_analyzer.analyze()


def get_time_multiplier():
    """المضاعف الزمني"""
    now = datetime.now(timezone.utc)
    rec = _time_analyzer._get_recommendation(now.hour)
