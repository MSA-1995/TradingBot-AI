...
# Fix OOM: كاش منفصل للشموع بحد اقصى
_ohlcv_cache = {}
_OHLCV_MAX_ENTRIES = 120
...
def get_market_data(exchange, ttl_hash=None):
    ...
    for market_coin in ['BTC/USDT', 'ETH/USDT', 'BNB/USDT']:
        try:
            ...
            print(f"Warning: Market data error {market_coin}: {e}")
            new_market_data[market_coin] = 0
...
def _fetch_ohlcv_cached(exchange, symbol, timeframe, cache_key, limit=None):
    ...
    try:
        _cv = _ohlcv_cache.get(cache_key, {})
        _now = time.time()
        ...
        if len(_ohlcv_cache) >= _OHLCV_MAX_ENTRIES:
            _oldest = min(_ohlcv_cache.items(), key=lambda x: x[1].get('ts', 0))
            del _ohlcv_cache[_oldest[0]]

        _ohlcv_cache[cache_key] = {'d': _pack_candles(_data), 'ts': _now}
        return _data
    except Exception as e:
        print(f"Warning: Failed to fetch {timeframe} candles for {symbol}: {e}")
        return None