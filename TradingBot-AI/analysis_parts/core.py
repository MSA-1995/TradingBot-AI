...
# ═══════════════════════════════════════════════════════════════
def get_market_analysis(exchange, symbol, limit=60, external_client=None):
    ...
    try:
        ...
        ohlcv_15m = _fetch_ohlcv_cached(exchange, symbol, '15m', f'{symbol}_15m', 30)
        ohlcv_1h = _fetch_ohlcv_cached(exchange, symbol, '1h', f'{symbol}_1h', 12)
        ohlcv_4h = _fetch_ohlcv_cached(exchange, symbol, '4h', f'{symbol}_4h', 14)
        ohlcv_1d = _fetch_ohlcv_cached(exchange, symbol, '1d', f'{symbol}_1d', 7)
        ...
        # Fix OOM: حذف الشموع الكبيرة
        for _key in ('candles', 'candles_5m', 'candles_15m', 'candles_1h', 'candles_4h', 'candles_1d', 'order_book'):
            analysis_dict.pop(_key, None)
