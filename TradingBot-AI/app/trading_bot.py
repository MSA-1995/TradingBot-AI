...
# Config
from app_config.config_encrypted import get_api_keys, get_discord_webhook
...
from messaging.discord import send_buy_notification, send_sell_notification, send_positions_report
...
from integrations.external_apis import ExternalAPIClient
from integrations.live_news_feed import start_live_news_feed, stop_live_news_feed
...
# Public exchange for live market data (no API key needed)
market_exchange = ccxt.binance({'enableRateLimit': True, 'options': {'defaultType': 'spot'}})
...
try:
    from ai.dl_client_v2 import DeepLearningClientV2
...
# ========== LIVE NEWS FEED ==========
try:
    start_live_news_feed()
except Exception as e:
    logging.error(f"Live news feed failed to start: {e}", exc_info=True)
    print(f"⚠️ Live news feed disabled: {e}")
...
def analyze_single_symbol(
    ...
    try:
        ...
        # Get analysis
        analysis = get_market_analysis(market_exchange, symbol)
        if not analysis:
            return None
