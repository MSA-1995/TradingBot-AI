...
class MemoryCache:
    ...
    def cleanup_if_needed(self, max_age_seconds: int = 900) -> int:
        """تنظيف دوري - يُستدعى دورياً لإزالة العناصر القديمة"""
        now = time.time()
        expired_keys = []
        ...
        for key, entry in self.cache.items():
            ...
            # احذف إذا انتهت صلاحيته أو عمره > max_age_seconds
            if (expiry is not None and now >= expiry) or age > max_age_seconds:
