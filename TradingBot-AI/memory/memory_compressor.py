...
class MemoryCompressor:
    ...
    def compress_candles(candles_data: list) -> Optional[dict]:
        ...
        try:
            cleaned = [
                ...
                if (normalized := MemoryCompressor._normalize_candle(c)) is not None
            ]
    ...
    def _normalize_candle(candle):
        """يدعم الشموع كـ list/tuple أو dict."""
        try:
            if isinstance(candle, dict):
                return [
                    int(candle.get('timestamp', candle.get('time', 0)) or 0),
            ...
            if len(candle) >= MemoryCompressor.MIN_CANDLE_FIELDS:
                return [
                    int(candle[0]),
                    float(candle[1]),
                    float(candle[2]),
