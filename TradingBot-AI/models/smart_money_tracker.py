...
class SmartMoneyTracker:
    ...
    def _get_whale_alerts(self, symbol):
        ...
        try:
            import requests
            # Whale Alert API (مجاني محدود، يحتاج API Key)
            api_key = "YOUR_WHALE_ALERT_API_KEY"  # استبدل بمفتاحك
    ...
    def get_confidence_adjustment(self, symbol, analysis):
        ...
        try:
            ...
            whale_alert_boost = self._get_whale_alerts(symbol) if hasattr(self, '_get_whale_alerts') and 'YOUR_WHALE_ALERT_API_KEY' not in open(__file__).read() else 0
