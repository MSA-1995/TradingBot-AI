        ...
        try:
            ...
            embed = {
                ...
                "fields": [
                    ...
                    {
                        "name": "Win Rate",
                        "value": f"{summary['win_rate']:.0f}%",
                        "inline": True
                    },
                    {
                        "name": "Total Profit",
                        "value": f"{summary['total_profit']:+.1f}%",
                        "inline": True
                    },
                    {
                        "name": "Average Profit",
                        "value": f"{summary['avg_profit_per_trade']:+.1f}%",
                        "inline": True
                    },
                    {
                        "name": "Trap Rate",
                        "value": f"{summary['trap_rate']:.0f}%",
                        "inline": True
                    },
                    {
                        "name": "Winning Trades",
            ...
            embed['fields'].append({
                "name": "Best Trade",
                "value": f"{best['symbol']}: {best['profit']:+.1f}%",
                "inline": True
            })
            embed['fields'].append({
                "name": "Worst Trade",
                "value": f"{worst['symbol']}: {worst['profit']:+.1f}%",
                "inline": True
            })
            ...
            if report['top_symbols']:
                top_text = "\n".join([
                    f"**{s['symbol']}**\nProfit: {s['total_profit']:+.1f}% | Win Rate: {s['win_rate']:.0f}%\n"
            ...
            if report['best_hours']:
                hours_text = "\n".join([
                    f"**{h['hour']}**\nProfit: {h['total_profit']:+.1f}% | Trades: {h['wins'] + h['losses']}\n"
