...
class AdaptiveIntelligence:
    ...
    def detect_market_regime(self, df) -> str:
        """كشف سلوك العملة المحلي (متذبذب/مستقر) - تم سحب صلاحية تحديد الاتجاه العام"""
        try:
            ...
            # تم إلغاء منطق الـ Bull/Bear هنا ليكون الماكرو هو المسؤول الوحيد
            # نركز فقط على التقلب (Volatility) لتعديل أحجام الصفقات وسلوك الـ AI
            if volatility > 4.5:
                return 'HIGH_VOLATILITY_LOCAL'
            elif volatility > 2.5:
                return 'NORMAL_VOLATILITY_LOCAL'
            else:
                return 'LOW_VOLATILITY_LOCAL'
