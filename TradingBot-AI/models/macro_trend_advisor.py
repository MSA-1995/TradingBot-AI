...
class MacroTrendAdvisor:
    ...
    def _analyze_symbol(self, symbol: str, df) -> dict:
        close = df['close']
        high = df['high']
        low = df['low']
        volume = df['volume']
    ...
    def _resolve_status(self, result: dict) -> str:
        """
        2/3 صاعدين + زخم إيجابي + حجم داعم → BULL
        2/3 هابطين + زخم سلبي              → BEAR
        غير ذلك                            → SIDEWAYS
        ...
        # القرار
        if bull >= 2 and avg_momentum > 0.3 and avg_volume >= 1.1:
            new_status = '🟢 BULL_MARKET' if bull >= 3 else '🟢 MILD_BULL'
        elif bear >= 2 and avg_momentum < -0.3:
            new_status = '🔴 BEAR_MARKET' if bear >= 3 else '🔴 MILD_BEAR'
        else:
            new_status = '⚪ SIDEWAYS'
