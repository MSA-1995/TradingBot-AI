...
class ExitStrategyModel:
    ...
    def _calculate_dynamic_wave_protection(self, profit_percent, analysis, history, drop_from_peak):
        ...
        try:
            ...
            volume_ratio = analysis.get('volume_ratio', 1.0)
            rsi = analysis.get('rsi', 50)
            whale_tracking_score = analysis.get('whale_confidence', 0)  # استخدم whale_confidence كـ whale_tracking_score
            sentiment_score = -4.8  # افتراضي

            # حساب ديناميكي كما في meta.py
            ...
            threshold = max(atr_p * 0.5, min(atr_p * 3.0, base_threshold))

            # تعديل حسب الربح الحالي (كما كان)
            if profit_percent > 2:
                pass  # استخدم threshold كما هو
            else:
                threshold *= 1.5  # للخسائر، اصبر أكثر

            # تعديل حسب التاريخ
            if history:
                avg_profit = history.get('avg_profit', 0)
                if profit_percent < avg_profit * 0.5:
                    threshold *= 1.3
