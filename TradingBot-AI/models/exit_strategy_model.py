...
class ExitStrategyModel:
    ...
    def should_exit(self, symbol, position, current_price, analysis, mtf):
        ...
        try:
            ...
            # 1. فحص التمسك بالموجة (Wave Rider Hold)
            # إذا كنا في ترند صاعد قوي (Shadow Alpha) لا نخرج لمجرد ربح بسيط قبل الـ 30%
            if analysis.get('relative_strength_btc', 0) > 1.0 and profit_percent < 30:
                return {'action': 'HOLD', 'reason': '💎 Wave Rider: Holding Shadow Alpha for 50%+'}
