...
import os
from app_config.config_encrypted import get_discord_webhook, get_critical_webhook
...
def send_positions_report(balance, invested, active_count, max_positions, open_positions=None, exchange=None):
    ...
    # Get current market status for macro points
    current_macro_status = "NEUTRAL"  # Default
    try:
        from analysis_parts.market_intelligence import get_market_regime
        # Try to get current market status
        current_macro_status = "NEUTRAL"  # Will be updated per position
    except Exception:
        pass

    def get_current_macro_points(macro_status):
        """Get current macro points based on market status"""
        from app_config.config import MACRO_BUY_POINTS, MACRO_SELL_POINTS
        return MACRO_SELL_POINTS.get(macro_status, 0)
    ...
    for i, (symbol, pos_data) in enumerate(sorted_positions):
        ...
        if exchange:
            try:
                from analysis_parts.core import get_market_analysis
        ...
        entry = (
            ...
            f"Macro Sell Points: {pos_data.get('macro_sell_points', 0):+d}\n\n"
        )
...
def send_advisor_report(signal_type, symbol, core_votes, meta_confidence,
    ...
    try:
        ...
        votes = core_votes or {}
        core_total = 0
        core_max = sum(max_pts for _, _, max_pts in advisor_config)

        # Build fields - same style as BUY/SELL
        fields = [
            {"name": "Pair",           "value": symbol,                        "inline": True},
            {"name": "Signal",         "value": signal_type.upper(),           "inline": True},
            {"name": "Meta Trading",   "value": f"{meta_pts:.1f}/{meta_max} pts ({meta_confidence:.0f}%)", "inline": True},
        ]
        ...
        # Core total
        fields.append({"name": "Core Total", "value": f"{core_total:.1f}/{core_max} pts", "inline": True})
        ...
        dynamic_pts  = support_data.get('dynamic_points', 0) if support_data else 0
        ...
        macro_str    = support_data.get('macro_strength', 'N/A') if support_data else 'N/A'
        fields.append({
            "name": "🌐 Macro Trend",
            "value": f"Status: {macro_curr} | Direction: {macro_dir} | Strength: {macro_str} | Key: {macro_key} | Macro: {macro_pts:+.0f} | Dyn: {dynamic_pts:+.1f}",
            "inline": False
        })
