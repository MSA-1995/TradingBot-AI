...
# ✅ استخدم الـ Singleton المشترك بدل كاش منفصل
from integrations.external_apis import get_global_external_client
try:
    from integrations.live_news_feed import get_live_news_data
except Exception:
    get_live_news_data = None
...
def get_sentiment_data(symbol=None, analysis=None):
    ...
    try:
        ...
        fng_data = client.get_market_sentiment_global()

        raw_fg          = fng_data.get('value', 50)
        sentiment_score = (raw_fg - 50) / 5.0

        result['sentiment_score']    = round(sentiment_score, 2)
        result['fear_greed_index']   = raw_fg
        result['fear_greed']         = raw_fg
        result['sentiment_velocity'] = 0.0
    ...
    # ========== 3. News Count من الكاش ==========
    try:
        if symbol:
            cached = get_live_news_data(symbol) if get_live_news_data else None
            if not cached:
                cache_key = f"news_{symbol}"
                cached    = _news_cache.get(cache_key) if MemoryCache else _news_cache.get(cache_key)
            if cached:
                result['positive_news_count'] = cached.get('positive', 0)
                result['negative_news_count'] = cached.get('negative', 0)
    except Exception:
        pass
...
class NewsAnalyzer:
    def __init__(self, storage=None):
        ...
        self.enabled = True if get_live_news_data else False

    def get_news_sentiment(self, symbol, hours=24):
        """Read live in-process news first, then fall back to storage."""
        if get_live_news_data:
            live_data = get_live_news_data(symbol, hours=hours)
            if live_data:
                return live_data
