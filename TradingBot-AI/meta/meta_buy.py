...
👑 meta_trading (40) + 11 Bottom Voters (40) + Support (20) = 100
"""
...
class BuyMixin:
    ...
    def _calculate_dynamic_macro_support(self,
        ...
        try:
            ...
            if cached:
                ...
                if isinstance(cached, dict):
                    ...
                    # non-dict cached value has no TTL - treat as expired
                    pass
        except Exception:
            pass
        ...
        try:
            ...
            if fg > 75:
                support -= 2.0
                details.append(f'🤑 Greed={fg:.0f}→-2')
            elif fg > 60:
                support -= 1.0
                details.append(f'😰 HighGreed={fg:.0f}→-1')
            elif fg < 25:
                support += 1.0
            ...
            def _ms(s):
                s_upper = str(s).upper()  # Fixed: ensure string conversion
        ...
        try:
            ...
            if hasattr(self, 'storage') and self.storage:
                try:
                    ...
                    logger.warning(f"Failed to save symbol memory for {symbol}: {e}")
    ...
    def should_buy(self, symbol: str, analysis: dict,
        ...
        try:
            ...
            if macro:
                ...
                ai['buy_mode']       = buy_mode.get('mode', 'UNKNOWN')
                ai['buy_mode_label'] = buy_mode.get('label', '')
        ...
        # ══════════════════════════════════════
        if buy_mode is None:
            from app_config.config import BUY_MODE_BALANCED
            buy_mode = BUY_MODE_BALANCED
        ...
        # ══════════════════════════════════════
        # 12 Bottom Voters = 40 Points
        # ══════════════════════════════════════
        core_votes = self._run_buy_core_voting(
            symbol, analysis_data, candles)

        # الأوزان الجديدة (12 مصوت = 40 نقطة)
        # Price Action: candle(7) + realtime_pa(7) + pattern(5) = 19
        # Trend: trend(2) + mtf(4) = 6
        # Volume: volume_forecast(1) + volume_pred(1) = 2
        # Smart Money: smart_money(2) + liquidity(2) = 4
        # Technical: fibonacci(3) + chart_cnn(1) = 4
        # Divergence: divergence(5) = 5  ← NEW (أقوى إشارة انعكاس)
        # المجموع = 19+6+2+4+4+5 = 40
        
        candle_points  = (core_votes.get('candle_expert',   0) / 100) * 7
        chart_points   = (core_votes.get('chart_cnn',       0) / 100) * 1
        rtpa_points    = (core_votes.get('realtime_pa',     0) / 100) * 7
        trend_points   = (core_votes.get('trend_detector',  0) / 100) * 2
        mtf_points     = (core_votes.get('multitimeframe',  0) / 100) * 4
        fib_points     = (core_votes.get('fibonacci',       0) / 100) * 3
        whale_points   = (core_votes.get('smart_money',     0) / 100) * 2
        volume_points  = (core_votes.get('volume_forecast', 0) / 100) * 1
        pattern_points = (core_votes.get('pattern',         0) / 100) * 5
        liq_points     = (core_votes.get('liquidity_vote',  0) / 100) * 2
        volp_points    = (core_votes.get('volume_pred',     0) / 100) * 1
        div_points     = (core_votes.get('divergence',      0) / 100) * 5  # NEW

        core_points = (
            candle_points + chart_points   + rtpa_points
            + trend_points + mtf_points    + fib_points     + whale_points
            + volume_points + pattern_points + liq_points
            + volp_points + div_points
        )
        ...
        if buy_points >= required:
            ...
            return {
                ...
                'reason'               : (
                    ...
                    f'Macro:{macro_points:+.0f} | '
                    f'Dyn:{dynamic_points:+.1f} | '
                    f'{buy_mode.get("label", "")}'),
