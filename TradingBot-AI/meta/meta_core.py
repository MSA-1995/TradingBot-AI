...
class Meta(AdvisorsMixin, BuyMixin, SellMixin, LearningMixin):
    ...
    def _calibrate_meta_confidence(self, confidence: float,
        ...
        try:
            ...
            # ── AdaptiveIntelligence adjustment ──
            try:
                adaptive = self.advisor_manager.get('AdaptiveIntelligence') if self.advisor_manager else None
                if adaptive:
                    symbol = analysis_data.get('symbol', '')
                    if symbol:
                        calibrated = adaptive.adjust_confidence(symbol, calibrated)
            except Exception:
                pass
    ...
    def _build_meta_features(self, rsi: float, macd_diff_pct: float,
        ...
        """بناء الميزات لنموذج meta_trading (49 ميزة)"""
        ...
        sell_signals = sum([
            ai.get('anomaly_risk',       0) > 70,  # Fixed: high risk = sell signal
