# bot/advisor_manager.py - السكرتير الذكي (نظام التحميل عند الطلب)
...
class AdvisorManager:
    ...
    def __init__(self, storage, capital_manager, exchange=None, dl_client=None):
        ...
        self._creators = {
            'DeepLearningClientV2': lambda: __import__('ai.dl_client_v2', fromlist=['DeepLearningClientV2']).DeepLearningClientV2(database_url=os.getenv('DATABASE_URL')) if os.getenv('DATABASE_URL') else None,
            'dl_client':            lambda: __import__('ai.dl_client_v2', fromlist=['DeepLearningClientV2']).DeepLearningClientV2(database_url=os.getenv('DATABASE_URL')) if os.getenv('DATABASE_URL') else None,
            ...
            'LiquidationShield':    lambda: self._create_liquidation_shield(),
    ...
    def _create_macro_advisor(self):\r\n        import threading\r\n        dl_client = self.__class__._advisors.get('dl_client') or self.__class__._advisors.get('DeepLearningClientV2')
        advisor = MacroTrendAdvisor(self._exchange, dl_client=dl_client)
        advisor.db = self._storage\r\n        if self._exchange:\r\n            advisor._fetch_thread_running = True\r\n            threading.Thread(target=advisor._bg_fetch, daemon=True).start()\r\n        return advisor\r\n\r\n    def _create_liquidation_shield(self):
        shield = LiquidationShield()
        shield.exchange = self._exchange
        return shield
...
class AIAdvisorWrapper:
    ...
    def get_confidence_adjustment(self, symbol, analysis, action='BUY'):
        ...
        try:
            advice = self.dl_client.get_advice(
                ...
                action=analysis.get('_action', action)
            )
