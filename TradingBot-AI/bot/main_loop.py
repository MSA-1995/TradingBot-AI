...
from trading.utils import get_active_positions_count, get_total_invested, should_send_report, format_price, save_open_positions
from messaging.discord import send_positions_report
from app_config.config import MAX_POSITIONS, LOOP_SLEEP, REPORT_INTERVAL, MAX_CAPITAL, BATCH_SIZE, MAX_WORKERS, META_DISPLAY_THRESHOLD

from bot.sell_handler import process_sell
from bot.buy_handler import process_buy
from app_config.config import MACRO_SELL_POINTS, MACRO_BUY_POINTS
...
def run_main_loop(exchange, ctx):
    ...
    while True:
      try:
        ...
        while True:
            loop_count   += 1
            _loop_start   = __import__('time').time()
            current_time  = __import__('datetime').datetime.now(__import__('datetime').timezone(__import__('datetime').timedelta(hours=3))).strftime('%H:%M:%S')
            # ========== BALANCE ==========
            ...
            try:
                ...
                if macro_advisor:
                    macro_status = macro_advisor.get_macro_status() or '⚪ SIDEWAYS'
            except Exception:
                pass
            print()
            print(f"✦•······················•✦•······················•✦")
            ...
            for result in active_results:
                ...
                if action == 'DISPLAY':
                    if result.get('confidence', 0) >= META_DISPLAY_THRESHOLD:
                        rsi  = analysis.get('rsi', result.get('rsi', 0))
                        vol  = analysis.get('volume_ratio', result.get('volume', 0))
                        macd = analysis.get('latest', {}).get('macd_diff_pct', analysis.get('macd_diff', 0))
                    ...
                    with symbols_data_lock:
                        for sym in symbols_to_fetch:
                            ...
                            if 'buy_price' in position and 'amount' in position:
                                ...
                                # حساب النقاط الخاصة بالماكرو للبيع من القاموس الفعلي
                                current_macro_points = MACRO_SELL_POINTS.get(current_macro_status, 0)
                                ...
                                open_positions_data[sym] = {
                                    ...
                                    'macro_sell_points':   current_macro_points  # النقاط المحدثة
                                }
                            else:
            ...
            # ========== CLEANUP ==========
            _loop_elapsed = __import__('time').time() - _loop_start
