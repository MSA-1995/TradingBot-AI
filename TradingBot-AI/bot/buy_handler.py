...
from trading.utils import execute_buy, save_open_positions
from messaging.discord import send_buy_notification, send_advisor_report
from app_config.config import MIN_TRADE_AMOUNT
...
def process_buy(result: dict, exchange, ctx: dict) -> bool:
    ...
    try:
        ...
        _points_match = re.search(r'Points:(\d+(?:\.\d+)?)', _reason)
        _total_points = (
            float(_points_match.group(1))
            if _points_match else result.get('confidence', 0)
        )
        ...
        _support_data = {
            ...
            'dynamic_points': _ai.get('dynamic_macro_points', 0),
