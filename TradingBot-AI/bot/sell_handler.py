...
from trading.utils import execute_sell, calculate_sell_value
from messaging.discord import send_sell_notification, send_advisor_report
...
def process_sell(result, exchange, ctx):
    ...
    try:
        ...
        if result.get('action') == 'SELL' and 'Points:' in _reason:
            ...
            _support_data = {
                ...
                'dynamic_points': _ai.get('dynamic_sell_points', 0),
            ...
            meta_confidence=_sell_votes.get('meta_trading', 0),
    ...
    try:
        ...
        try:
            ...
            if buy_time_str:
                buy_time = datetime.fromisoformat(str(buy_time_str))
                now      = datetime.now(buy_time.tzinfo or timezone.utc)
                hours_held = (now - buy_time).total_seconds() / 3600
        except Exception:
            pass
