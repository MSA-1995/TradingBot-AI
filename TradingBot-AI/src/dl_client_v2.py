    ...
    def get_advice(self, rsi, macd, volume_ratio, price_momentum, confidence=50,
        ...
        if model:
            ...
            features = (base_features + [whale, inflow, whale_wallet_changes,
                        institutional_accumulation, smart_money_ratio, exchange_whale_flows, 0.5, 0.5])[:12]
        ...
        if model:
            ...
            features = (base_features + [rsi, analysis.get('atr', 1), volatility_risk_score,
                        ...
                        behavioral_risk, systemic_risk, 0.5, 0.5])[:12]
            names    = (base_names + ['risk_rsi', 'risk_atr', 'volatility_risk_score',
                        'correlation_risk', 'gap_risk_score', 'black_swan_probability',
                        'behavioral_risk', 'systemic_risk', 'tp_accuracy', 'sell_accuracy'])[:12]
        ...
        if model:
            ...
            features = (base_features + [analysis.get('anomaly_score', 0), statistical_outliers,
                        pattern_anomalies, behavioral_anomalies, volume_anomalies, 0.5, 0.5])[:12]
            names    = (base_names + ['anomaly_score', 'statistical_outliers', 'pattern_anomalies',
                        'behavioral_anomalies', 'volume_anomalies', 'tp_accuracy', 'sell_accuracy'])[:12]
        ...
        if model:
            ...
            features = (base_features + [price_momentum, harmonic_patterns_score, elliott_wave_signals,
                        fractal_patterns, cycle_patterns, momentum_patterns, 0.5, 0.5])[:12]
            names    = (base_names + ['pattern_momentum', 'harmonic_patterns_score', 'elliott_wave_signals',
        ...
        if model:
            ...
            features = [
                ...
                analysis.get('price_change_24h_old', 0), rsi, macd
            ]
            names = [
