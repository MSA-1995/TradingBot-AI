...
# يبقى مشغولاً وقت طويل ويُحسب كـ idle من طرف الـ server
    ...
    def get_advice(self, rsi, macd, volume_ratio, price_momentum, confidence=50,
        ...
        def _get_prediction_advice(advisor_name, current_analysis_data):
            model = self._models.get(advisor_name)
            if not model:
                return "N/A"

            try:
                # استخدام دالة استخلاص الميزات من البوت
                from features_extractor import calculate_enhanced_features
                features_vector = calculate_enhanced_features(current_analysis_data, None)
                ...
                print(f"⚠️ {advisor_name} prediction error: {e}")
                return "N/A"
        ...
        advice['sentiment'] = _get_prediction_advice('sentiment', analysis)
        advice['crypto_news'] = _get_prediction_advice('crypto_news', analysis)
    ...
    def learn_from_trade(self, profit, trade_quality, buy_votes, sell_votes, signal_type='sell'):
        """
        تعلم المستشارين من الصفقة (للتوافق مع sell_handler)
        هذه الدالة موجودة للتوافق فقط - التعلم الفعلي يتم عبر التدريب الدوري
        """
        try:
            # التعلم يتم عبر حفظ الصفقات في الداتابيز وإعادة التدريب دورياً
            pass
        except Exception:
            pass
