...
class DeepLearningClientV2:
    ...
    def _predict_buy(self, model_name, features, feature_names):
        ...
        try:
            ...
            #print(f"   🔍 {model_name}: proba={score:.3f} thr={threshold} -> {vote}")
            return vote
        except Exception as e:
    ...
    def vote_buy_now(self, rsi, macd, volume_ratio, price_momentum, confidence=50,
        ...
        def _rule_buy(advisor_name, ml_pred):
            """
            إذا النموذج غير محمل (None) أو معطوب → نستخدم القواعد.
            النموذج المعطوب = يصوت 1 دائماً لـ risk/anomaly (false alarm)
            أو يصوت 0 دائماً لـ exit/pattern/liquidity (dead model)
            """
            _model_votes_log[advisor_name] = ml_pred

            if ml_pred is not None:
                # كشف النماذج المعطوبة:
                # risk/anomaly يصوتون 1 دائماً = false alarm → تجاهل
                if advisor_name in ('risk', 'anomaly') and ml_pred == 1:
                    # إذا RSI مقبول وVol مقبول، لا تصدق الـ model
                    rsi_ok = rsi < rsi_buy_threshold
                    not_extreme = not (rsi < 15 or rsi > 85 or volume_ratio > 4.0)
                    if rsi_ok and not_extreme:
                        pass  # نكمل للـ rule-based بدل الـ model
                    else:
                        return ml_pred  # خطر حقيقي → نثق بالنموذج
                # exit/pattern/liquidity يصوتون 0 دائماً = dead model → تجاهل
                elif advisor_name in ('exit', 'pattern', 'liquidity') and ml_pred == 0:
                    pass  # نكمل للـ rule-based
                else:
                    return ml_pred  # نموذج يعطي نتائج متنوعة → نثق به
            # Rule-based fallback حسب طبيعة كل مستشار
            rsi_ok = rsi < rsi_buy_threshold
            macd_ok = (macd > 0) if not macd_required else (macd > 0.3)
            vol_ok  = volume_ratio >= 1.0
            if advisor_name == 'exit':
                # مستشار الخروج: يشتري إذا RSI منخفض + حجم جيد
                return 1 if (rsi_ok and vol_ok) else 0
            elif advisor_name == 'risk':
                # مستشار المخاطر: متحفظ - يحتاج RSI + MACD
                return 1 if (rsi_ok and macd_ok) else 0
            elif advisor_name == 'pattern':
                # مستشار الأنماط: يصوت بإشارة الشمعة أو RSI+MACD
                candle_signal = is_bottom_candle or (candle_analysis or {}).get('is_reversal', False)
                return 1 if (candle_signal or (rsi_ok and macd_ok and vol_ok)) else 0
            elif advisor_name == 'anomaly':
                # مستشار الشذوذ: يصوت ضد الشراء فقط عند تقلب عالٍ جداً
                extreme = rsi < 15 or rsi > 85 or volume_ratio > 4.0
                return 0 if extreme else (1 if rsi_ok else 0)
            elif advisor_name == 'liquidity':
                # مستشار السيولة: يصوت بالحجم
                return 1 if vol_ok else 0
            return 0
    ...
    def vote_sell_now(self, rsi, macd, volume_ratio, price_momentum, liquidity_metrics=None, candle_analysis=None, market_sentiment=None):
        ...
        def _rule_sell(advisor_name, ml_pred):
            """إذا النموذج غير محمل (None)، يصوت بالقواعد بدلاً من الصفر."""
            if ml_pred is not None:
                return ml_pred
            rsi_high   = rsi >= 68
            macd_neg   = macd < -0.3
            vol_low    = volume_ratio < 0.8
            peak_sig   = is_peak_candle or is_rejection
            if advisor_name == 'exit':
                return 1 if (rsi_high or peak_sig) else 0
            elif advisor_name == 'risk':
                return 1 if (rsi_high and (macd_neg or vol_low)) else 0
            elif advisor_name == 'pattern':
                return 1 if (peak_sig or (rsi_high and macd_neg)) else 0
            elif advisor_name == 'anomaly':
                extreme = rsi > 82 or volume_ratio > 3.5
                return 1 if (extreme or (rsi_high and vol_low)) else 0
            elif advisor_name == 'liquidity':
                return 1 if vol_low else 0
            return 0
