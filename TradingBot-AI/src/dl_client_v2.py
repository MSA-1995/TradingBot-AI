...
class DeepLearningClientV2:
    def __init__(self, database_url, load_models=True):
        ...
        self._feature_names = {}
        self._prediction_cache = {}  # Cache مضغوط للتنبؤات
        try:
            self._connect_db()
            if load_models:
    ...
    def get_advice(self, rsi, macd, volume_ratio, price_momentum, confidence=50,
        ...
        def _predict(model, features, names, is_sell=False):
            # ✅ إنشاء مفتاح cache مضغوط (معطل لتحسين السرعة)
            # cache_key = zlib.compress(pickle.dumps((features, names, is_sell)), level=9)
            # if cache_key in self._prediction_cache:
            #     return pickle.loads(zlib.decompress(self._prediction_cache[cache_key]))

            try:
                X = pd.DataFrame([features], columns=names)
                # تعطيل فحص الشكل لتجنب خطأ عدم تطابق عدد الميزات
                ...
                # ✅ حفظ مضغوط في cache (معطل لتحسين السرعة)
                # compressed_result = zlib.compress(pickle.dumps(result), level=9)
                # self._prediction_cache[cache_key] = compressed_result

                return result
            except Exception as e:
                return "N/A"
        
        # Smart Money - Match training features (12)
        ...
        if model:
            ...
            features = base_features + [whale, inflow, whale_wallet_changes, institutional_accumulation, smart_money_ratio, exchange_whale_flows, 0.5, 0.5]
            names = base_names + ['whale_activity', 'exchange_inflow', 'whale_wallet_changes', 'institutional_accumulation', 'smart_money_ratio', 'exchange_whale_flows', 'tp_accuracy', 'sell_accuracy']
            # Match training: use first 12 features
        ...
        # Risk - Match training features (12)
        ...
        if model:
            ...
            systemic_risk = analysis.get('systemic_risk', 0.0)
            features = base_features + [rsi, analysis.get('atr', 1), volatility_risk_score, correlation_risk, gap_risk_score, black_swan_probability, behavioral_risk, systemic_risk, 0.5, 0.5]
            names = base_names + ['risk_rsi', 'risk_atr', 'volatility_risk_score', 'correlation_risk', 'gap_risk_score', 'black_swan_probability', 'behavioral_risk', 'systemic_risk', 'tp_accuracy', 'sell_accuracy']
            # Match training: use first 12 features
            features = features[:12]
            names = names[:12]
        ...
        # Anomaly - Match training features (12)
        ...
        if model:
            ...
            features = base_features + [analysis.get('anomaly_score', 0), statistical_outliers, pattern_anomalies, behavioral_anomalies, volume_anomalies, 0.5, 0.5]
            names = base_names + ['anomaly_score', 'statistical_outliers', 'pattern_anomalies', 'behavioral_anomalies', 'volume_anomalies', 'tp_accuracy', 'sell_accuracy']
            # Match training: use first 12 features
            features = features[:12]
            names = names[:12]
        ...
        # Exit - Match training features (12)
        ...
        if model:
            ...
            features = base_features + [24, profit_optimization_score, time_decay_signals, opportunity_cost_exits, market_condition_exits, 0.5, 0.5]
            names = base_names + ['hours_held', 'profit_optimization_score', 'time_decay_signals', 'opportunity_cost_exits', 'market_condition_exits', 'tp_accuracy', 'sell_accuracy']
            # Match training: use first 12 features
            features = features[:12]
            names = names[:12]
        ...
        # Pattern - Match training features (12)
        ...
        if model:
            ...
            features = base_features + [price_momentum, harmonic_patterns_score, elliott_wave_signals, fractal_patterns, cycle_patterns, momentum_patterns, 0.5, 0.5]
            names = base_names + ['pattern_momentum', 'harmonic_patterns_score', 'elliott_wave_signals', 'fractal_patterns', 'cycle_patterns', 'momentum_patterns', 'tp_accuracy', 'sell_accuracy']
            # Match training: use first 12 features
            features = features[:12]
            names = names[:12]
        ...
        # Chart CNN - Match training features (12)
        ...
        if model:
            ...
            features = base_features + [bullish, bearish, neutral, attention_mechanism_score, multi_scale_features, temporal_features, 0.5, 0.5]
            names = base_names + ['bullish_chart', 'bearish_chart', 'neutral_chart', 'attention_mechanism_score', 'multi_scale_features', 'temporal_features', 'tp_accuracy', 'sell_accuracy']
            # Match training: use first 12 features
            features = features[:12]
            names = names[:12]
        ...
        # Volume Pred - Match training features (12)
        model = self._models.get('volume_pred')
        if model:
            vol = analysis.get('volume', 0)
            volume_trend_strength = analysis.get('volume_trend_strength', 0)
            volume_seasonality = analysis.get('volume_seasonality', 0)
            volume_correlation = analysis.get('volume_correlation', 0)
            features = [
            ...
            names = [
                ...
                'price_change_24h_old', 'rsi', 'macd', 'atr', 'bid_ask_spread',
                'volume_momentum', 'volume_acceleration', 'volume_trend_strength', 'volume_seasonality', 'volume_correlation',
        ...
        # ✅ تنظيف cache التنبؤات بعد الانتهاء (معطل لأن cache معطل)
