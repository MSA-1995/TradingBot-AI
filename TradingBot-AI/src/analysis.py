...
def analyze_reversal(df, rsi):
    """
    تحليل "قاع الدورة الكبرى" - استراتيجية صيد الموجات (110 نقطة)
    - الشموع والأنماط السيادية: 40 نقطة
    - تدفق السيولة المؤسسية: 30 نقطة (مهم جداً للـ 80%)
    - RSI + MACD (فريمات كبيرة): 20 نقطة
    ...
    try:
        ...
        # البحث في مدى الشموع المحدد في Config لاكتشاف تجميع القاع الحقيقي
        for i in range(2, min(REVERSAL_CANDLES, len(df))):
        ...
        # --- 3. Market Structure & Accumulation - 20 نقطة ---
        ...
        # --- 4. تدفق السيولة المؤسسية (Volume Spikes) - 30 نقطة ---
        vol_score = 0
        current_vol_ratio = df.iloc[-1].get('volume_ratio', 1.0)
        ...
        score_breakdown['institutional_volume'] = vol_score
        ...
        # --- 6. Volume + Divergence - 15 نقطة ---
        # انفجار الحجم عند القاع = تأكيد قوي للانعكاس
        vol_score = 0
        if len(df) >= 2:
            current_vol = df.iloc[-1].get('volume_ratio', 1)
            # فحص إضافي: هل الانفجار مصحوب بشمعة خضراء (تأكيد اتجاه)
            last_candle = df.iloc[-1]
            is_bullish_vol = last_candle['close'] > last_candle['open']
            if current_vol > 3.0:
                vol_score = 15  # انفجار حجم قوي جداً = إشارة قاع قوية
                reasons.append(f"Vol Explosion {current_vol:.1f}x (+15)")
            elif current_vol > 2.0:
                vol_score = 10
                reasons.append(f"Vol Very High {current_vol:.1f}x (+10)")
...
def analyze_peak(df, rsi):
    ...
    try:
        ...
        # --- 1. الشموع والأنماط - 45 نقطة (قمة حقيقية) ---
        candle_signal = False
        pattern_score = 0
        pattern_name = ""
        pattern_found_idx = -1
        
        n = min(REVERSAL_CANDLES, len(df)) # البحث في نفس المدى للقمم أيضاً
        ...
        for i in range(2, min(n, len(df))):
            ...
            if is_shooting_star:
                ...
                pattern_found_idx = i
                reasons.append(f"{pattern_name} (+45)")
                break
        ...
        # --- 2. RSI + MACD - 25 نقطة (محسّن - قمة حقيقية) ---
        rsi_score = 0
        if rsi > 78:  # ✅ قمة قوية جداً
            rsi_score = 20
            reasons.append(f"RSI Extreme ({rsi:.0f}) (+20)")
        elif rsi > 73:  # ✅ قمة قوية
            rsi_score = 15
            reasons.append(f"RSI Very High ({rsi:.0f}) (+15)")
        elif rsi > 68:  # ✅ قمة متوسطة
            rsi_score = 10
            reasons.append(f"RSI Overbought ({rsi:.0f}) (+10)")
        elif rsi > 63:  # ✅ قمة خفيفة
            rsi_score = 5
            reasons.append(f"RSI High ({rsi:.0f}) (+5)")
        else:
            rsi_score = 0
        ...
        # --- 6. Volume + Divergence - 15 نقطة ---
        # انفجار الحجم عند القمة مع شمعة حمراء = تأكيد قوي للبيع
        vol_score = 0
        if len(df) >= 2:
            current_vol = df.iloc[-1].get('volume_ratio', 1)
            last_candle = df.iloc[-1]
            ...
            if current_vol > 3.0 and is_bearish_vol:
                vol_score = 15  # انفجار حجم هابط = إشارة قمة قوية
                reasons.append(f"Vol Explosion Bearish {current_vol:.1f}x (+15)")
            elif current_vol > 2.0:
                vol_score = 10
                reasons.append(f"Vol Very High {current_vol:.1f}x (+10)")
            elif current_vol > 1.5:
