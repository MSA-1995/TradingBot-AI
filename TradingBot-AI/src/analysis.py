...
def analyze_mtf_confirmation(df):
    ...
    closes = df['close'].tolist()
    rsi_col = df['rsi'] if 'rsi' in df.columns else None
    ...
    try:
        ...
        analysis_dict = {
            ...
            **liquidity_metrics,
            ...
            'panic_greed': detect_panic_greed({
                'volume': float(latest['volume']) if latest['volume'] == latest['volume'] else 0,
                'volume_sma': float(latest['volume_sma']) if latest['volume_sma'] == latest['volume_sma'] else 0,
                'price_momentum': float(latest['price_change']) if latest['price_change'] == latest['price_change'] else 0,
            }),
            ...
            **get_sentiment_data(symbol, {'close': latest['close'], 'rsi': latest['rsi'], 'volume_ratio': float(latest.get('volume_sma', 1) and latest['volume'] / max(latest.get('volume_sma', latest['volume']), 1)), 'btc_change_1h': btc_change_1h})
        }
