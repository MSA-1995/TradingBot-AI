...
def get_market_analysis(exchange, symbol, limit=120, external_client=None):
    ...
    try:
        ...
        # ─────────────────────────────────────────────
        # Fix 1: chart_cnn_score
        # ─────────────────────────────────────────────
        analysis_dict['chart_cnn_score'] = min(
            analysis_dict.get('attention_mechanism_score', 0) * 100, 100
        )
        ...
        # Chart CNN
        analysis_dict['attention_mechanism_score'] = min(latest['volume_ratio'] / 1.5, 1.0)
        analysis_dict['multi_scale_features']      = abs(latest['macd_diff']) * 100
        analysis_dict['temporal_features']         = abs(latest['price_change_1h']) / 5.0

        # ✅ تحديث chart_cnn_score
        analysis_dict['chart_cnn_score'] = min(
            analysis_dict['attention_mechanism_score'] * 100, 100
        )
