...
_order_book_cache = {}  # {symbol: {'data': {}, 'timestamp': 0}}
_impact_cache = {}  # {symbol: {'data': {}, 'ts': 0}}
...
def get_market_analysis(exchange, symbol, limit=120, external_client=None):
    ...
    try:
        ...
        try:
            _ck = f"{symbol}_5m"
            _cv = _order_book_cache.get(_ck, {})
            if __import__('time').time() - _cv.get('ts', 0) < 240:
                ohlcv_5m = _cv['d']
            else:
                ohlcv_5m = exchange.fetch_ohlcv(symbol, '5m', limit=limit)
                _order_book_cache[_ck] = {'d': ohlcv_5m, 'ts': __import__('time').time()}
        except Exception as e:
        ...
        try:
            _ck = f'{symbol}_15m'
            _cv = _order_book_cache.get(_ck, {})
            if __import__('time').time() - _cv.get('ts', 0) < 840:
                ohlcv_15m = _cv['d']
            else:
                ohlcv_15m = exchange.fetch_ohlcv(symbol, '15m', limit=60)
                _order_book_cache[_ck] = {'d': ohlcv_15m, 'ts': __import__('time').time()}
        ...
        try:
            _ck = f'{symbol}_1h'
            _cv = _order_book_cache.get(_ck, {})
            if __import__('time').time() - _cv.get('ts', 0) < 3540:
                ohlcv_1h = _cv['d']
            else:
                ohlcv_1h = exchange.fetch_ohlcv(symbol, '1h', limit=24)
                _order_book_cache[_ck] = {'d': ohlcv_1h, 'ts': __import__('time').time()}
        ...
        try:
            _ck = f'{symbol}_4h'
            _cv = _order_book_cache.get(_ck, {})
            if __import__('time').time() - _cv.get('ts', 0) < 14340:
                ohlcv_4h = _cv['d']
            else:
                ohlcv_4h = exchange.fetch_ohlcv(symbol, '4h', limit=30)
                _order_book_cache[_ck] = {'d': ohlcv_4h, 'ts': __import__('time').time()}
        ...
        try:
            _ck = f'{symbol}_1d'
            _cv = _order_book_cache.get(_ck, {})
            if __import__('time').time() - _cv.get('ts', 0) < 86340:
                ohlcv_1d = _cv['d']
            else:
                ohlcv_1d = exchange.fetch_ohlcv(symbol, '1d', limit=14)
                _order_book_cache[_ck] = {'d': ohlcv_1d, 'ts': __import__('time').time()}
