...
class StorageManager:
    ...
    def get_news_data(self, symbol=None):
        ...
        if data is not None and self._last_news_update is not None:
            if (now - self._last_news_update).total_seconds() < 1800:
                return data

        # ✅ لو ما لقى بالكاش الخاص، جرب news_all
        if symbol and data is None:
            all_news = self.mem_cache.get("news_all")
            if all_news is not None and self._last_news_update is not None:
                if (now - self._last_news_update).total_seconds() < 1800:
                    return all_news
        ...
        try:
            ...
            if result:
                ...
                self.mem_cache.set(cache_key, result, expiry_seconds=1800)
            return result
