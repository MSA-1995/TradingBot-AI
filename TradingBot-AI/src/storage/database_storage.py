...
class DatabaseStorage:
    def __init__(self):
        ...
        self.pool = ThreadedConnectionPool(20, 20, **self._db_params) # تم رفع الحد الأقصى للاتصالات
    ...
    def _create_tables(self):
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                """)
                # فهرس لتحسين تقارير الصفقات المفتوحة وفحص الصفقات القديمة
                ...
                """)
                # ✅ فهارس لتحسين سرعة البحث والترتيب في تاريخ الصفقات
                ...
                # فهارس لاسترجاع أفضل الأنماط نجاحاً حسب النوع
                ...
                # 6. Models Index (تحسين سرعة تشغيل البوت وفحص التحديثات)
                # يساعد في جلب أحدث نسخة من الموديل بسرعة فائقة
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_models_lookup ON dl_models_v2(model_name, trained_at DESC);")

                # ========== فهارس إضافية محسّنة ==========
                
                # 7. فهرس مركب لـ symbol_memory (أهم واحد!)
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_symbol_memory_symbol_updated ON symbol_memory(symbol, last_updated DESC);")
                
                # 8. فهرس لـ trade_quality في trades_history
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_trades_quality ON trades_history(trade_quality, timestamp DESC);")
                
                # 9. فهرس لـ profit في trades_history (للإحصائيات)
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_trades_profit ON trades_history(profit DESC) WHERE profit IS NOT NULL;")
                
                # 10. فهرس مركب لـ symbol + timestamp في trades_history
    ...
    def load_setting(self, key):
        """تحميل إعداد محدد من جدول bot_settings"""
        conn = None
        try:
            conn = self._get_conn()
            with conn.cursor() as cursor:
            ...
            if conn: conn.rollback(); self._put_conn(conn)
    ...
    def save_trade(self, trade_data):
        ...
        try:
            ...
            def convert_to_native(d):
                if isinstance(d, float) and not math.isfinite(d): return 0.0
                if isinstance(d, np.generic):
                    val = d.item()
                    return 0.0 if isinstance(val, float) and not math.isfinite(val) else val
                if isinstance(d, dict): return {k: convert_to_native(v) for k, v in d.items()}
                if isinstance(d, list): return [convert_to_native(i) for i in d]
                return d
            
            trade_data = convert_to_native(trade_data if isinstance(trade_data, dict) else {})
            conn = self._get_conn()
            with conn.cursor() as cursor:
                cursor.execute("""
    ...
    def save_positions_batch(self, positions_data):
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                def convert_to_native(d):
                    for k, v in d.items():
                        if isinstance(v, float) and not math.isfinite(v): d[k] = 0.0
                        elif hasattr(v, 'item'): d[k] = v.item()
                        elif isinstance(v, dict): convert_to_native(v)
                    return d
                valid_positions = [convert_to_native(p.copy()) for p in positions_data if isinstance(p, dict)]
                for pos in valid_positions:
                    if 'data' in pos and isinstance(pos['data'], dict): pos['data'] = self.json.dumps(pos['data'])
                sql_upsert = """
                    ...
                    ON CONFLICT (symbol) DO UPDATE SET buy_price = EXCLUDED.buy_price, amount = EXCLUDED.amount, 
                    highest_price = EXCLUDED.highest_price, tp_level_1 = EXCLUDED.tp_level_1, tp_level_2 = EXCLUDED.tp_level_2, 
                    buy_time = EXCLUDED.buy_time, invested = EXCLUDED.invested, data = EXCLUDED.data;
                """
    ...
    def load_positions(self):
        ...
        try:
            ...
            with conn.cursor(cursor_factory=self.RealDictCursor) as cursor:
                ...
                return {row['symbol']: {
                    'buy_price': row['buy_price'], 'amount': row['amount'], 'highest_price': row['highest_price'],
                    'tp_level_1': row['tp_level_1'], 'tp_level_2': row['tp_level_2'], 'buy_time': row['buy_time'].isoformat(),
    ...
    def save_pattern(self, pattern_data):
        ...
        try:
            conn = self._get_conn()
            with conn.cursor() as cursor:
                cursor.execute("INSERT INTO learned_patterns (pattern_type, data, success_rate) VALUES (%s, %s, %s)", 
    ...
    def load_all_patterns(self):
        ...
        try:
            ...
            with conn.cursor(cursor_factory=self.RealDictCursor) as cursor:
                cursor.execute("SET TRANSACTION READ ONLY;")
                cursor.execute("SELECT id, pattern_type, (data->'features') as features, (data->>'success_rate')::float as success_rate FROM learned_patterns ORDER BY last_updated DESC;")
                return [{'id': r['id'], 'pattern_type': r['pattern_type'], 'data': {'features': r.get('features', {})}, 'success_rate': r['success_rate']} for r in cursor.fetchall()]
        except Exception:
            return []
        finally:
            if conn: conn.rollback(); self._put_conn(conn)
    ...
    def cleanup_old_data(self):
        ...
        try:
            conn = self._get_conn()
            with conn.cursor() as cursor:
                queries = {"trades (30d)": ("trades_history", "timestamp", 30), "patterns (7d)": ("learned_patterns", "last_updated", 7), "symbol (180d)": ("symbol_memory", "last_updated", 180)}
                for key, (table, date_col, days) in queries.items():
                    while True:
                        cursor.execute(f"DELETE FROM {table} WHERE ctid IN (SELECT ctid FROM {table} WHERE {date_col} < NOW() - INTERVAL '{days} days' LIMIT 500);")
                        if cursor.rowcount == 0: break
                        conn.commit(); time.sleep(0.1)
            return True
        except Exception:
            if conn: conn.rollback()
            return False
    ...
    def update_symbol_memory(self, symbol, profit, trade_quality, hours_held, rsi, volume_ratio, sentiment=0, whale_conf=0, panic=0, optimism=0,
                             profit_loss_ratio=0, volume_trend=0, psychological_summary='Neutral', courage_boost=0, time_memory_modifier=0, 
                             pattern_score=0, win_rate_boost=0):
        """تحديث ذاكرة العملة بعد كل صفقة"""
        conn = None
        try:
            # ✅ تحويل المدخلات لمنع خطأ "schema np does not exist"
            profit, hours_held = float(profit), float(hours_held)
            rsi, volume_ratio = float(rsi), float(volume_ratio)
            sentiment, whale_conf = float(sentiment), float(whale_conf)
            panic, optimism = float(panic), float(optimism)
            ...
            with conn.cursor() as cursor:
                cursor.execute("""
                    INSERT INTO symbol_memory (symbol, total_trades, win_count, loss_count, trap_count, avg_profit, max_profit, min_profit, avg_hold_hours, best_rsi, best_volume_ratio, last_trade_quality, sentiment_avg, whale_confidence_avg, profit_loss_ratio, volume_trend, panic_score_avg, optimism_penalty_avg, psychological_summary, courage_boost, time_memory_modifier, pattern_score, win_rate_boost, last_updated)
                    VALUES (%s, 1, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NOW())
                    ON CONFLICT (symbol) DO UPDATE SET total_trades = symbol_memory.total_trades + 1, win_count = symbol_memory.win_count + EXCLUDED.win_count,
                    loss_count = symbol_memory.loss_count + EXCLUDED.loss_count, trap_count = symbol_memory.trap_count + EXCLUDED.trap_count,
                    avg_profit = (symbol_memory.avg_profit * symbol_memory.total_trades + EXCLUDED.avg_profit) / (symbol_memory.total_trades + 1),
                    max_profit = GREATEST(symbol_memory.max_profit, EXCLUDED.max_profit), min_profit = LEAST(symbol_memory.min_profit, EXCLUDED.min_profit),
                    ...
                    best_rsi = EXCLUDED.best_rsi, best_volume_ratio = EXCLUDED.best_volume_ratio, last_trade_quality = EXCLUDED.last_trade_quality,
                    ...
                    last_updated = NOW();
                """, (symbol, 1 if profit > 0 else 0, 1 if profit <= 0 else 0, 1 if trade_quality in ['TRAP', 'RISKY'] else 0, profit, profit, profit, hours_held, rsi, volume_ratio, trade_quality, sentiment, whale_conf, profit_loss_ratio, volume_trend, panic, optimism, psychological_summary, courage_boost, time_memory_modifier, pattern_score, win_rate_boost))
            conn.commit()
            return True
        except Exception as e:
