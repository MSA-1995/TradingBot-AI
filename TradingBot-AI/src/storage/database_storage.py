...
class DatabaseStorage:
    def __init__(self):
        ...
        # minconn=2, maxconn=10 (Pre-create fewer connections to reduce startup time)
        self.pool = ThreadedConnectionPool(2, 10, **self._db_params)
        # --- END ---
        ...
        # Symbol memory cache
        self.symbol_memory_cache = {}
        self._load_symbol_memory_cache()
        self.cache_update_timer = threading.Timer(30*60, self._update_symbol_memory_cache)
        self.cache_update_timer.start()
    ...
    def save_trade(self, trade_data):
        ...
        try:
            ...
            """, (
                ...
                # rsi, volume_ratio, trade_quality, profit — يطابق ترتيب الأعمدة في SQL (مواضع 8-11)
    ...
    def get_symbol_memory(self, symbol):
        """جلب ذاكرة الملك للعملة من cache"""
        return self.symbol_memory_cache.get(symbol)

    def _load_symbol_memory_cache(self):
        """تحميل جميع symbol_memory في cache عند التشغيل"""
        conn = None
        try:
            conn = self._get_conn()
            ...
            rows = cursor.fetchall()
            cursor.close()
            conn.commit()  # ✅ ضروري: يُنهي الـ transaction ويُرجع الـ connection لحالة IDLE
            for row in rows:
                self.symbol_memory_cache[row['symbol']] = row
        except Exception as e:
            print(f"Error loading symbol memory cache: {e}")
            if conn:
                try:
                    conn.rollback()
                except Exception:
    ...
    def _update_symbol_memory_cache(self):
        """تحديث cache كل 30 دقيقة"""
        self._load_symbol_memory_cache()
        # إعادة جدولة التايمر
        self.cache_update_timer = threading.Timer(30*60, self._update_symbol_memory_cache)
        self.cache_update_timer.start()
