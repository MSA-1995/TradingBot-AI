...
class DatabaseStorage:
    ...
    def update_trade_sentiment(self, trade_id, sentiment_data):
        """تحديث بيانات المشاعر لصفقة محددة"""
        conn = None
        try:
            conn = self._get_conn()
            with conn.cursor() as cursor:
                cursor.execute("""
                    ...
                    SET sentiment_score = %s, panic_score = %s, optimism_penalty = %s
                    WHERE id = %s
                """, (sentiment_data['sentiment_score'], sentiment_data['panic_score'], 
                      sentiment_data.get('optimism_penalty', 0), trade_id))
            conn.commit()
            return True
        except Exception:
            if conn: conn.rollback()
