...
class Meta:
    ...
    def should_buy(self, symbol, analysis, models_scores=None, candles=None, preloaded_advisors=None):
        ...
        try:
            ...
            # طباعة النصائح لميتا
            print(f"🧠 Advisors Advice for {symbol}: {advisors_advice}")

            # ميتا يقرر بناءً على النصائح (هنا نفس المنطق السابق، لكن بدون تصويت)
            # للتبسيط، نحتفظ بالمنطق، لكن ميتا يرى النصائح
            total_advisors = len(advisors_advice)
            # افتراض أن بعض النصائح تشير للشراء
            buy_signals = sum(1 for adv in advisors_advice.values() if 'buy' in adv.lower() or 'low' in adv.lower())
            buy_vote_count = buy_signals  # للحفاظ على المتغير
    ...
    def should_sell(self, symbol, position, current_price, analysis, mtf, candles=None, preloaded_advisors=None):
        ...
        # --- حذف الستوب لوس الثابت، ميتا يتحكم في السمارت ستوب لوس ---
        ...
        try:
            ...
            if dl_client and hasattr(dl_client, 'vote_sell_now'):
                ...
                sell_votes, market_status = dl_client.get_advice(  # استخدام get_advice للبيع أيضاً
                    rsi=rsi, macd=macd_diff, volume_ratio=volume_ratio,
    ...
    def _get_market_mood(self, analysis):
        ...
        if up_count >= 2:
            ...
            mood_details['min_votes_needed'] = 0   # ميتا يقرر بدون تصويت
            mood_details['total_advisors'] = 10
        elif down_count >= 2:
