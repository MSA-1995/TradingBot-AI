...
class Meta:
    ...
    def should_buy(self, symbol, analysis, models_scores=None, candles=None, preloaded_advisors=None):
        ...
        try:
            ...
            votes_required = 3 if market_status == 'bullish' else 4 if market_status == 'neutral' else 5
        ...
        # ✅ قرار الملك: هل أريد الشراء؟ (3 شروط - متوسط)
        ...
        # الشرط 3 (متوسط): RSI منخفض + حجم جيد = فرصة حقيقية
        elif rsi <= 40 and volume_ratio >= 1.2 and temp_conf >= MIN_CONFIDENCE - 10:
            king_wants_to_buy = True
            king_reason = f"King: Oversold+Vol ({rsi:.0f}, {volume_ratio:.1f}x)"
    ...
    def should_sell(self, symbol, position, current_price, analysis, mtf, candles=None, preloaded_advisors=None):
        ...
        trigger_activated = (
            ...
            (rsi >= 70 and profit_percent >= 0.8) or  # ✅ متوسط: خففنا من 72+1.0 إلى 70+0.8
            (profit_percent >= 3.0)                    # ✅ متوسط: ربح ممتاز يُفعّل دائماً
        )
        ...
        urgent_sell = sell_conf >= MIN_SELL_CONFIDENCE + 10 or rsi >= 90  # ✅ خففنا من 73 إلى 71
