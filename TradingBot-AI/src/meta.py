...
class Meta:
    ...
    def should_buy(self, symbol, analysis, models_scores):
        ...
        # Check if we have the model data blueprint
        if self.meta_learner_data is None:
            return {'action': 'DISPLAY', 'reason': 'Meta-Learner blueprint NOT LOADED', 'confidence': 0}
    ...
    def should_sell(self, symbol, position, current_price, analysis, mtf):
        ...
        # 1. Fixed 2% Trailing Stop Loss
        stop_loss_percent = 2.0
        reason_prefix = "TRAILING STOP"

        # The drop is calculated from the highest price achieved
