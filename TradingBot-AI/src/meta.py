...
class Meta:
    def __init__(self, advisor_manager=None, storage=None):
        ...
        try:
            ...
            print("✅ Meta: Multi-Timeframe Analyzer initialized")
        except Exception as e:
            print(f"⚠️ Meta: Failed to load Multi-Timeframe Analyzer: {e}")
    ...
    def should_buy(self, symbol, analysis, models_scores=None, candles=None, preloaded_advisors=None):
        ...
        # 🆕 توقعات Meta للعملة والسوق عند القاع
        coin_forecast = {'direction': 'neutral', 'confidence': 50}  # صعود/نزول/محايد
        market_forecast = {'direction': 'neutral', 'confidence': 50}  # صعود/انهيار/محايد
        ...
        if self.meta_model:
            try:
                ...
                # 🔮 Meta يتوقع العملة: هل بتصعد؟
                if buy_probability > 0.65:
                    coin_forecast = {'direction': 'bullish', 'confidence': min(95, confidence)}
                elif buy_probability < 0.45:
                    coin_forecast = {'direction': 'bearish', 'confidence': min(95, 100 - confidence)}
                else:
                    coin_forecast = {'direction': 'neutral', 'confidence': 50}
                
                # 🌍 Meta يتوقع السوق الكلي: هل بيصعد/ينهار/محايد؟
                ...
                # حساب توقع السوق من المؤشرات
                market_score = (macro_trend * 0.5) + (sentiment_score * 5) + (whale_activity * 0.3)
                
                if market_score > 60:
                    market_forecast = {'direction': 'bullish', 'confidence': min(95, market_score)}
                elif market_score < 40:
                    market_forecast = {'direction': 'bearish', 'confidence': min(95, 100 - market_score)}
                else:
                    market_forecast = {'direction': 'neutral', 'confidence': 50}
        ...
        # 🎯 تأثير التوقعات على قرار الشراء (ديناميكي)
        forecast_boost = 0
        if coin_forecast['direction'] == 'bullish' and market_forecast['direction'] == 'bullish':
            # كلاهما إيجابي - رفع الثقة
            forecast_boost = (coin_forecast['confidence'] + market_forecast['confidence']) / 20
        elif coin_forecast['direction'] == 'bearish' or market_forecast['direction'] == 'bearish':
            # أحدهما سلبي - خفض الثقة
            penalty = (coin_forecast['confidence'] + market_forecast['confidence']) / 20
            forecast_boost = -penalty
        
        confidence += forecast_boost
        ...
        # 🆕 Multi-Timeframe Analysis - كشف القاع السريع (الطبقة 1)
        ...
        if self.mtf_analyzer and candles and len(candles) >= 5:
            try:
                ...
                volume_data_1h = [c.get('volume', 0) for c in candles_1h[-10:]] if len(candles_1h) >= 10 else None
                ...
                realtime_bottom = self.mtf_analyzer.analyze_bottom(
    ...
    def should_sell(self, symbol, position, current_price, analysis, mtf, candles=None, preloaded_advisors=None):
        ...
        # 🆕 توقعات Meta للعملة والسوق عند القمة
        coin_forecast_sell = {'direction': 'neutral', 'confidence': 50}  # بتصعد أكثر أو بتنزل؟
        market_forecast_sell = {'direction': 'neutral', 'confidence': 50}  # السوق بيصعد/ينهار/محايد؟
        
        # 🆕 Multi-Timeframe Analysis - كشف القمة السريع (الطبقة 1)
        realtime_peak = None
        if self.mtf_analyzer and candles and len(candles) >= 5:
            try:
                # جلب الشموع من 3 أطر زمنية
                ...
                # استخدام Multi-Timeframe للكشف الدقيق
                realtime_peak = self.mtf_analyzer.analyze_peak(
                    candles_5m=candles_5m,
                    candles_15m=candles_15m,
                    candles_1h=candles_1h,
                ...
                print(f"⚠️ Multi-timeframe peak detection error: {e}")
        ...
        if self.meta_model:
            try:
                ...
                # 🔮 Meta يتوقع العملة عند القمة: هل بتصعد أكثر أو بتنزل؟
                if sell_probability > 0.65:
                    coin_forecast_sell = {'direction': 'bearish', 'confidence': min(95, sell_confidence)}
                elif sell_probability < 0.45:
                    coin_forecast_sell = {'direction': 'bullish', 'confidence': min(95, 100 - sell_confidence)}
                else:
                    coin_forecast_sell = {'direction': 'neutral', 'confidence': 50}
                
                # 🌍 Meta يتوقع السوق الكلي عند القمة: هل بيصعد/ينهار/محايد؟
                macro_trend_sell = advisors_intelligence.get('macro_trend_sell', 50)
                sentiment_score = advisors_intelligence.get('sentiment_score', 0)
                whale_tracking_score = advisors_intelligence.get('whale_tracking_score', 0)
                
                # حساب توقع السوق من المؤشرات
                market_score_sell = (macro_trend_sell * 0.5) + (sentiment_score * -5) + (whale_tracking_score * -0.3)
                
                if market_score_sell > 60:
                    market_forecast_sell = {'direction': 'bullish', 'confidence': min(95, market_score_sell)}
                elif market_score_sell < 40:
                    market_forecast_sell = {'direction': 'bearish', 'confidence': min(95, 100 - market_score_sell)}
                else:
                    market_forecast_sell = {'direction': 'neutral', 'confidence': 50}
        ...
        # 🎯 تأثير التوقعات على قرار البيع (ديناميكي)
        forecast_boost_sell = 0
        if coin_forecast_sell['direction'] == 'bearish' and market_forecast_sell['direction'] == 'bearish':
            # كلاهما سلبي - رفع ثقة البيع
            forecast_boost_sell = (coin_forecast_sell['confidence'] + market_forecast_sell['confidence']) / 20
        elif coin_forecast_sell['direction'] == 'bullish' or market_forecast_sell['direction'] == 'bullish':
            # أحدهما إيجابي - خفض ثقة البيع
            penalty = (coin_forecast_sell['confidence'] + market_forecast_sell['confidence']) / 20
            forecast_boost_sell = -penalty
        
        sell_confidence += forecast_boost_sell
        ...
        if sell_confidence >= MIN_SELL_CONFIDENCE:
            ...
            return {
                'action': 'SELL',
                'reason': f'🤖 Meta AI: {sell_confidence:.1f}% sell confidence (REAL PEAK detected)',
                'profit': profit_percent,
                'optimism_penalty': 0,
                'sell_votes': {},
                'sell_confidence': sell_confidence,
                'coin_forecast': coin_forecast_sell,
                'market_forecast': market_forecast_sell
            }
        else:
            gc.collect()
            return {
        ...
        sentiment_score = advisors_intelligence.get('sentiment_score', 0)
        volume_ratio = analysis.get('volume_ratio', 1.0)
        peak_score = advisors_intelligence.get('peak_score', 50)

        # 🛡️ حساب Stop Loss الديناميكي
        base_threshold = atr * 2.5
        min_threshold = risk_level / 25
        base_threshold = max(min_threshold, base_threshold)
        risk_modifier = (risk_level - 50) / 100
        base_threshold += risk_modifier * 3
        whale_modifier = whale_tracking_score / 200
        base_threshold -= whale_modifier * 2
        sentiment_modifier = max(-10, min(10, sentiment_score)) / 100
        base_threshold += sentiment_modifier * 2
        trailing_threshold = max(1.0, min(15.0, base_threshold))
        
        # 🆕 توقعات Meta عند Stop Loss: هل أبيع أو أنتظر؟
        stop_coin_forecast = {'direction': 'neutral', 'confidence': 50}
        stop_market_forecast = {'direction': 'neutral', 'confidence': 50}
        
        # Meta يتوقع: هل العملة بترتفع أو بتنزل أكثر؟
        if drop_from_peak >= trailing_threshold * 0.7:  # قريب من Stop Loss
            try:
                # توقع العملة بناءً على المؤشرات
                if rsi < 30 and volume_ratio > 2.0:
                    stop_coin_forecast = {'direction': 'bullish', 'confidence': 70}  # قد ترتفع
                elif rsi > 70 or macd_diff < -2:
                    stop_coin_forecast = {'direction': 'bearish', 'confidence': 75}  # ستنزل أكثر
                else:
                    stop_coin_forecast = {'direction': 'neutral', 'confidence': 50}
                
                # توقع السوق الكلي
                macro_bear = advisors_intelligence.get('macro_bear_signal', 0)
                if macro_bear == 1:
                    stop_market_forecast = {'direction': 'bearish', 'confidence': 80}  # السوق بينهار
                elif advisors_intelligence.get('macro_trend_sell', 50) < 30:
                    stop_market_forecast = {'direction': 'bearish', 'confidence': 70}
                elif advisors_intelligence.get('macro_trend_sell', 50) > 70:
                    stop_market_forecast = {'direction': 'bullish', 'confidence': 70}
                else:
                    stop_market_forecast = {'direction': 'neutral', 'confidence': 50}
                
                # 🎯 تأثير التوقعات على Stop Loss (ديناميكي)
                if stop_coin_forecast['direction'] == 'bullish' and stop_market_forecast['direction'] == 'bullish':
                    # كلاهما إيجابي - توسيع Stop Loss (انتظر)
                    multiplier = 1 + ((stop_coin_forecast['confidence'] + stop_market_forecast['confidence']) / 200)
                    trailing_threshold *= multiplier
                elif stop_coin_forecast['direction'] == 'bearish' and stop_market_forecast['direction'] == 'bearish':
                    # كلاهما سلبي - تضييق Stop Loss (بيع أسرع)
                    multiplier = 1 - ((stop_coin_forecast['confidence'] + stop_market_forecast['confidence']) / 200)
                    trailing_threshold *= max(0.5, multiplier)  # حد أدنى 0.5 للأمان
                
            except Exception as e:
                print(f"⚠️ Stop loss forecast error: {e}")
        ...
        if drop_from_peak >= trailing_threshold:
            ...
            return {
                ...
                'sell_votes': sell_votes,
                'peak_score': peak_score,
                'stop_loss_threshold': trailing_threshold,
                'coin_forecast': stop_coin_forecast,
                'market_forecast': stop_market_forecast
            }
