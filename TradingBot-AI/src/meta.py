...
class _EnhancedPatternRecognitionFallback:
    """Fallback محلي عندما لا يكون EnhancedPatternRecognition متاحاً من DB"""
    ...
    def analyze_peak_hunter_pattern(self, candles: list) -> dict:
        try:
            ...
            position    = (last_close - recent_low) / price_range if price_range > 0 else 0.5
            ...
            lower_highs = highs[-1] < highs[-3] < highs[-5] if len(highs) >= 5 else False
...
class Meta:
    ...
    def _gather_pattern_intelligence(self, symbol: str,
        ...
        try:
            ...
            if pattern_rec is None or not hasattr(pattern_rec, 'analyze_peak_hunter_pattern'):
    ...
    def should_buy(self, symbol: str, analysis: dict,
        ...
        try:
            if self.mtf_analyzer and candles and len(candles) >= 3:
                ...
                if result:
                    ...
                    if mtf_conf > 20:
                        mtf_points = (mtf_conf / 100) * 8 * (max(conf, 1) / 3)
                        if result.get('is_bottom'):
                            print(f"⚡ MTF Bottom [{symbol}]: {mtf_conf:.0f}% ({conf}/3 TF)")
        ...
        if 'BULL' in macro_clean:
            macro_points = 2
        elif 'NEUTRAL' in macro_clean:
            macro_points = 1
        elif 'BEAR' in macro_clean:
            macro_points = 0
        ...
        try:
            if hasattr(self, 'news_analyzer') and self.news_analyzer:
                ...
                if news_positive > 0 or news_negative > 0:
                    ...
                    news_points = max(0, (sent_score / 10) * 2) if sent_score > 0 else 0
        except Exception as e:
            print(f"⚠️ News points error [{symbol}]: {e}")

        news_points = min(news_points, 4)
    ...
    def should_sell(self, symbol: str, position: dict,
        ...
        from config import MIN_SELL_CONFIDENCE, MIN_SELL_PROFIT, get_prediction_modes
        ...
        # =========================================================
        # نظام النقاط لكشف القمة (111 → مطبّعة على 100)
        # =========================================================
        ...
        if 'BULL' in macro_status:
            required_sell_points = 75
        elif 'BEAR' in macro_status:
            required_sell_points = 55
        else:
            required_sell_points = 65
    ...
    def _smart_sell_check(self, symbol: str, position: dict,
                          profit_pct: float, sell_mode: dict) -> Optional[dict]:
        ...
        # ── SNIPER_PROFIT: Lock max profit & exit ──
        if mode == 'SNIPER_PROFIT':
            if profit_pct > tracker['highest_profit']:
                tracker['highest_profit'] = profit_pct
                ...
                # ✅ Protection: Don't sell at loss in SNIPER_PROFIT
                if profit_pct < 0:
                    tracker['last_profit'] = profit_pct
                    self._smart_sell_tracker[price_key] = tracker
                    return None
        ...
        # ── SNIPER_EXIT: Minimize loss & exit ──
        ...
        # ── CAUTIOUS: Watch and wait ──
            ...
            if profit_pct > tracker.get('highest_profit', profit_pct):
                ...
                # ✅ Protection: Don't sell at loss in CAUTIOUS mode
                if profit_pct < 0:
                    tracker['last_profit'] = profit_pct
                    self._smart_sell_tracker[price_key] = tracker
                    return None  # Wait instead of selling at loss!
