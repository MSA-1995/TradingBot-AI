...
class Meta:
    ...
    def should_sell(self, symbol, position, current_price, analysis, mtf, candles=None, preloaded_advisors=None):
        ...
        # --- ✅ احسب كل النقاط أولاً حتى يكون السكور دقيقاً في رسالة HOLD ---
        ...
        # =========================================================
        # 🏔️ كشف القمة الحقيقية — يتجاوز MetaModel مباشرة
        # إذا analyze_peak أثبت قمة حقيقية (candle_signal=True)
        # + ربح موجب + RSI مرتفع → نكمل للتصويت بدون انتظار MetaModel
        # =========================================================
        is_real_peak = (
            candle_condition and          # شمعة قمة حقيقية (Shooting Star / Bearish Engulfing / Evening Star / Three Crows / Dark Cloud)
            peak_score >= 35 and          # نقاط كافية تثبت القمة (مو مجرد نقاط بسيطة)
            profit_percent >= 0.5 and     # في ربح (ما نبيع بخسارة)
            rsi >= 55                     # RSI مرتفع يؤكد القمة (مو في قاع)
        )

        if is_real_peak:
            print(f"🏔️ REAL PEAK DETECTED [{symbol}] | CandleSignal={candle_condition} | PeakScore={peak_score:.0f} | RSI={rsi:.0f} | Profit={profit_percent:+.2f}% — Bypassing MetaModel gate")
            sell_conf += 25  # دفعة قوية للسكور تعكس ثقة القمة
            sell_reasons.append(f"RealPeak(+25,score={peak_score:.0f})")
            # نكمل للتصويت مباشرة — لا نرجع HOLD هنا

        else:
            # =========================================================
            # 👑 قرار الملك: هل نبيع؟ (بعد حساب كل النقاط للسكور الدقيق)
            # =========================================================
        ...
        # --- ✅ الحد الأدنى للربح (بعد قرار الملك أو القمة الحقيقية)
        ...
        # Bullish → صعب بيع (4/5) | Neutral → متوسط (3/5) | Bearish → سهل بيع (2/5)
        market_mood_for_sell = mood_details.get('mood', 'Neutral')
        if market_mood_for_sell == 'Bullish':
            min_votes_needed = 4   # سوق صاعد: يلزم 4/5 للبيع (صعب)
        elif market_mood_for_sell == 'Bearish':
            min_votes_needed = 2   # سوق هابط: يكفي 2/5 للبيع (سهل)
        else:
            min_votes_needed = 3   # محايد: 3/5
