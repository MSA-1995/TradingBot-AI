...
class Meta:
    ...
    def should_sell(self, symbol, position, current_price, analysis, mtf, candles=None, preloaded_advisors=None):
        ...
        # --- 🚨 1. الحماية الفورية (Flash Crash & Emergency) ---
        flash_crash = analysis.get('flash_crash_protection', {})
        flash_risk = flash_crash.get('risk_score', 0)
        
        # بيع طوارئ إذا زاد الخطر فجأة عن 60% (بدون شروط ربح)
        if flash_risk >= 60:
            return {
                'action': 'SELL',
        ...
        # --- 🛡️ 2. السمارت ستوب لوس الديناميكي (Smart Stop Loss) ---
        # الملك يقرر رقم الستوب لوس بناءً على القراءات والنصائح فقط
        try:
            dl_client = self.advisor_manager.get('dl_client')
            # جلب النصائح الحالية لتحليلها
            current_advices = dl_client.get_advice(
            ...
            # حساب الستوب لوس بناءً على تقلبات العملة (ATR)
            atr = analysis.get('atr', 0)
            # القاعدة: الستوب لوس الافتراضي هو ضعف التقلب اليومي (ATR) كنسبة مئوية
            dynamic_sl_base = -(atr / current_price * 100) * 2.0 if current_price > 0 else -2.0
            
            # تعديل الرقم بناءً على نصائح المستشارين
            sl_adjustment = 0
            for adv in current_advices.values():
                msg = adv.lower()
                if 'high risk' in msg or 'high' in msg: sl_adjustment += 0.5  # تقريب الستوب لوس (حذر)
                if 'poor' in msg: sl_adjustment += 0.3                       # تقريب الستوب لوس
                if 'bullish' in msg: sl_adjustment -= 0.4                    # إعطاء مساحة (ثقة)
            
            smart_sl_threshold = dynamic_sl_base + sl_adjustment
            
            if profit_percent <= smart_sl_threshold:
                return {'action': 'SELL', 'reason': f'🛡️ Smart Stop Loss: {profit_percent:.2f}% (Limit: {smart_sl_threshold:.2f}%)', 'profit': profit_percent}
        except Exception as e:
            print(f"⚠️ Smart SL calculation error: {e}")
        ...
        try:
            ...
            if dl_client and hasattr(dl_client, 'vote_sell_now'):
                ...
                sell_votes = dl_client.get_advice(  # ✅ إصلاح: get_advice ترجع قاموساً واحداً فقط
                    rsi=rsi, macd=macd_diff, volume_ratio=volume_ratio,
                ...
                # ✅ نفس المنطق في البيع: 10 مستشارين فقط
                total_advisors = 10
                active_advices = sum(1 for k, v in sell_votes.items() if k != 'meta_trading' and v and str(v) != "N/A") if sell_votes else 0
                sell_vote_count = active_advices
                vote_breakdown = sell_votes if sell_votes else {}
        ...
        # ✅ القرار للملك (صائد القمم) فقط
        if trigger_activated and sell_conf >= MIN_SELL_CONFIDENCE:
            action = 'SELL'
            reason = f"SELL | Score:{sell_conf}/110 | Advices:{sell_vote_count}/{total_advisors} | {', '.join(sell_reasons[:2])}"
        else:
            action = 'HOLD'
            reason = f"Hold | Score:{sell_conf}/110 | Advices:{sell_vote_count}/{total_advisors}"
    ...
    def _calculate_smart_amount(self, symbol, confidence, analysis):
        ...
        try:
            # القاعدة الأساسية للمبلغ ($12 - $23) بناءً على ثقة الملك
            base_amount = 15.0
            confidence_factor = (confidence - 50) / 50.0  # نطاق من -1.0 إلى 1.0
            final_amount = base_amount + (confidence_factor * 8.0)
