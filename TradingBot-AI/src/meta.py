...
class Meta:
    ...
    def should_buy(self, symbol, analysis, models_scores=None, candles=None, preloaded_advisors=None):
        ...
        # =========================================================
        # 🚀 ADVANCED LEVEL UP FILTERS (الخيارات الخمسة)
        # =========================================================
        # 1. تدقيق تدفق السيولة (Order Flow Imbalance)
        ob_imbalance = analysis_data.get('order_book_imbalance', 0)
        if ob_imbalance > 0.6: # طلبات الشراء أقوى بـ 60%
            temp_conf += 15
            reasons.append(f"OrderFlow-Bullish({ob_imbalance:+.1f})")
        elif ob_imbalance < -0.4:
            temp_conf -= 10
            reasons.append("OrderFlow-Weak")

        # 2. الحماية من الكسر الوهمي والضوضاء (Volatility Filtering)
        if analysis_data.get('is_noise', False):
            temp_conf -= 20
            reasons.append("Signal-Noise-Filtered")
        if analysis_data.get('fake_break', False):
            temp_conf -= 30
            reasons.append("🚫 Fake-Break-Detected")

        # 3. ربط الارتباطات (Market Correlations)
        btc_dom = analysis_data.get('external_impact', {}).get('btc_dominance', 50)
        btc_trend = analysis_data.get('btc_change_1h', 0)
        if btc_trend < -1.5 and symbol != 'BTC/USDT': # السوق ينهار
            temp_conf -= 40
            reasons.append("Market-Correlation-Risk")

        # 5. التحليل الزمني المتعدد (MTF Alignment)
        mtf_trend = analysis_data.get('mtf', {}).get('trend', 'neutral')
        if mtf_trend == 'bearish' and rsi < 40: # محاولة شراء في ترند هابط كبير
            temp_conf -= 25
            reasons.append("MTF-Trend-Conflict")
        elif mtf_trend == 'bullish':
            temp_conf += 10
            reasons.append("MTF-Alignment-Bonus")
