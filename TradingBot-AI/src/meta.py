...
class Meta:
    ...
    def should_sell(self, symbol, position, current_price, analysis, mtf, candles=None, preloaded_advisors=None):
        """القرار الاستراتيجي: حماية رأس المال (Option A - 15-20% Target)"""
        ...
        if drop_from_peak >= trailing_threshold:
            return {
                'action': 'SELL',
                'reason': f'Capital Protection Exit: -{drop_from_peak:.1f}% from peak',
        ...
        return {
            'action': 'HOLD', 
            'reason': f'Capital Protection | Profit: {profit_percent:+.1f}% | Peak: {peak_score}/110', 
            'profit': profit_percent
        }
