...
class Meta:
    ...
    def should_sell(self, symbol, position, current_price, analysis, mtf, candles=None, preloaded_advisors=None):
        ...
        # تحسين كشف القمة بالمشاعر والزخم بدون تعقيد
        sentiment_score = analysis.get('sentiment_score', 0)
        price_momentum = analysis.get('price_momentum', 0)

        if sentiment_score < -3:
            peak_confidence += 10  # أكثر احتمال للبيع إذا المشاعر سلبية
            peak_reasons.append("Sentiment Bearish (+10)")

        if price_momentum < -0.5:
            peak_confidence += 15  # زخم هابط
            peak_reasons.append("Negative Momentum (+15)")
        ...
        elif peak_confidence >= 35:  # إنهاك وانهيار من المستشارين
            king_wants_to_sell = True
