...
class Meta:
    ...
    def should_sell(self, symbol, position, current_price, analysis, mtf, candles=None, preloaded_advisors=None):
        ...
        if atr_p < 1.8: # 🟢 السوق هادئ (نضيق الوقف لحماية الأرباح من أي تذبذب غادر)
            trailing_threshold = 7.0
        elif atr_p > 4.0: # 🔴 السوق "مجنون" (نوسع الوقف لإعطاء العملة "نفس" لتجاوز التذبذب العالي)
            trailing_threshold = 17.5
        else: # ⚪ تقلب طبيعي
            trailing_threshold = 12.0

        # تعديل إضافي للموجات الكبرى: إذا كان الربح > 50%، نوسع الوقف قليلاً لمنع الخروج المبكر
        if profit_percent > 50:
            trailing_threshold = max(trailing_threshold, 15.0)
    ...
    def learn_from_trade(self, profit, trade_quality, buy_votes, sell_votes, symbol=None, position=None, extra_data=None):
        ...
        try:
            ...
            if profit > 5.0: # لا نعتبر الشراء "ناجحاً جداً" للتعلم إلا إذا حقق موجة 5% فأكثر (Wave Rider)
                ...
                pos_keywords = ['Safe', 'Bullish', 'Sweep', 'Alpha', 'Wall', 'Institutional']
