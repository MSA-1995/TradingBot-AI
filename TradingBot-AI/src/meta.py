...
class Meta:
    ...
    def should_buy(self, symbol, analysis, models_scores, candles=None):
        ...
        if pattern_analyzer and candles and len(candles) >= 10:
            peak_hunter_result = pattern_analyzer.analyze_peak_hunter_pattern(candles[-10:])
    ...
    def should_sell(self, symbol, position, current_price, analysis, mtf, candles=None):
        ...
        if dl_client:
            try:
                ...
                highest_price = position.get('highest_price', buy_price)
                highest_profit_percent = ((highest_price - buy_price) / buy_price) * 100 if buy_price > 0 else 0
                drop_from_high_percent = ((highest_price - current_price) / buy_price) * 100 if buy_price > 0 else 0
