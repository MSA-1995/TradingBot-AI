...
def process_buy(result, exchange, ctx):
    ...
    # ✅ إصلاح حرجي: حفظ الصفقة فوراً في قاعدة البيانات والذاكرة
    # لضمان عدم ضياع العملة إذا فشلت الخطوات التالية (الديسكورد أو السيولة)
    ...
    try:
        ...
        print(f"✅ Position saved to DB for {symbol}")
    except Exception as e:
        print(f"❌ DATABASE CRITICAL ERROR: Could not save {symbol}: {e}")

    # ------------------------------------------------------------------
    # الآن يمكن معالجة التنبيهات والبيانات الإضافية (لو فشلت لن تضيع الصفقة)
    # ------------------------------------------------------------------
    ...
    if 'decision' in result:
        ...
        # إضافة whale_confidence إذا متاح
        advisor_scores['whale_confidence'] = result.get('analysis', {}).get('whale_confidence', 0)
