...
# ──────────────────────────────────────────────
# 🔧 Helper: Calculate TP Dynamically
# SL is handled dynamically by meta_sell
# ──────────────────────────────────────────────
def _calculate_tp_sl(price: float, confidence: float) -> tuple[float, float]:
    """
    Calculates Take Profit dynamically based on confidence.
    - confidence 0->100
    - TP: from 2% to 6% based on confidence
    - SL: handled by meta_sell dynamic system (ATR based)
    """
    confidence = max(0.0, min(100.0, confidence))
    tp_pct     = 0.02 + (confidence / 100.0) * 0.04   # 2% - 6%
    tp_target  = round(price * (1 + tp_pct), 6)
    sl_target  = round(price * (1 - tp_pct * 0.5), 6) # placeholder only - meta_sell overrides
...
def process_buy(result: dict, exchange, ctx: dict) -> bool:
    ...
    if _FIB_AVAILABLE:
        try:
            ...
            high     = analysis.get('high_24h', price * 1.05)
            low      = analysis.get('low_24h',  price * 0.95)
            fib_data = {'high_24h': high, 'low_24h': low, 'rsi': rsi}
            at_res, _ = fib.is_at_resistance(price, fib_data, tolerance=1.0, volume_ratio=vol_ratio, symbol=symbol)
            if at_res:
                print(f'☠️ BUY REJECTED [{symbol}]: Price at Fibonacci Resistance')
                return False
            if rsi > 75:
                print(f'☠️ BUY REJECTED [{symbol}]: RSI={rsi:.1f} Overbought')
                return False
        except Exception as e:
            print(f'⚠️ [{symbol}] Fibonacci filter error: {e}')
