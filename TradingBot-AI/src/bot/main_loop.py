        ...
        while True:
            ...
            for result in active_results:
                ...
                if action == 'HOLD':
                    ...
                    print(f"{line_color}{profit_emoji} {symbol:12} {format_price(result['price'])} | Profit:{profit:+7.1f}% | Buy:{format_price(result.get('buy_price', 0))} | High:{format_price(result.get('highest', 0))} | {result['reason']}{Style.RESET_ALL}")
                    continue
                    ...
                    with symbols_data_lock:
                        for sym in symbols_to_fetch:
                            ...
                            if 'buy_price' in position and 'amount' in position:
                                ...
                                # جلب بيانات التصويت
                                decision_factors = position.get('decision_factors', {})
                                ...
                                open_positions_data[sym] = {
                                    ...
                                    'buy_confidence': decision_factors.get('buy_vote_percentage', 0),
                                    'buy_votes':      advisor_votes,
