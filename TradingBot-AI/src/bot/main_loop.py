...
from config import MAX_POSITIONS, LOOP_SLEEP, REPORT_INTERVAL, MAX_CAPITAL, BATCH_SIZE, MAX_WORKERS, META_DISPLAY_THRESHOLD

from bot.sell_handler import process_sell
from bot.buy_handler import process_buy
from config import MACRO_SELL_POINTS, MACRO_BUY_POINTS
                        ...
                        for sym in symbols_to_fetch:
                            ...
                            if 'buy_price' in position and 'amount' in position:
                                ...
                                try:
                                    ...
                                    if 'BULL' in str(market_regime):
                                        current_macro_status = "BULL"
                                    elif 'BEAR' in str(market_regime):
                                ...
                                # حساب النقاط الخاصة بالماكرو للبيع
                                current_macro_points = 0
                                if current_macro_status == "BULL":
                                    current_macro_points = -10  # سوق صاعد = نقاط سلبية للبيع
                                elif current_macro_status == "BEAR":
                                    current_macro_points = +10  # سوق هابط = نقاط موجبة للبيع
