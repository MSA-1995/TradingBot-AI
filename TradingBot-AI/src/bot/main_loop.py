        ...
        while True:
            ...
            for result in active_results:
                ...
                if action == 'HOLD':
                    ...
                    # 🆕 حفظ Stop Loss في position
                    with symbols_data_lock:
                        if symbol in SYMBOLS_DATA and SYMBOLS_DATA[symbol].get('position'):
                            SYMBOLS_DATA[symbol]['position']['stop_loss_threshold'] = result.get('stop_loss_threshold', 6.0)
                    ...
                    print(f"{line_color}{profit_emoji} {symbol:12} {format_price(result['price'])} | Profit:{profit:+7.1f}% | Buy:{format_price(result.get('buy_price', 0))} | High:{format_price(result.get('highest', 0))} | {result['reason']}{forecast_text}{Style.RESET_ALL}")
                    continue
                ...
                if action == 'STRONG':
                    ...
                    print(f"{Fore.GREEN}💪 {result['symbol']:12} {format_price(result['price'])} | RSI:{result['rsi']:>5.1f} | Vol:{vol_status} {min(result['volume'], 100):.0f}x | MACD:{result['macd']:>+6.1f} | Conf:{result['confidence']:.0f}/100{news_display} | {result.get('reason', '')}{forecast_text}{Style.RESET_ALL}")
                    continue
                ...
                if action == 'DISPLAY':
                    if result.get('confidence', 0) >= META_DISPLAY_THRESHOLD:
                        ...
                        print(f"📊 {symbol:12} ${result['price']:>8.2f} | RSI:{result['rsi']:>5.1f} | Vol:{vol_status} {min(result['volume'], 100):.0f}x | MACD:{result['macd']:>+6.1f} | Conf:{result['confidence']:.0f}/100{news_display} | {result.get('reason', '')}{forecast_text}")
                    continue
            ...
            if should_send_report(last_report_time, REPORT_INTERVAL):
                    ...
                    with symbols_data_lock:
                        for sym in symbols_to_fetch:
                            ...
                            if 'buy_price' in position and 'amount' in position:
                                ...
                                # 🔄 جلب بيانات التصويت من data (JSON)
                                ...
                                # ✅ قراءة البيانات مباشرة من position_data
                                buy_confidence = position_data.get('buy_confidence', 0)
                                advisor_votes = position_data.get('advisor_votes', {})
                                ai_data = position_data.get('ai_data', {})
                                # ✅ قراءة stop_loss_threshold من position (الكاش) أو من data (الداتابيز)
                                stop_loss_threshold = position.get('stop_loss_threshold') or position_data.get('stop_loss_threshold')
                ...
                last_report_time = datetime.now(timezone.utc) # Update time after sending
