...
import sys
import os
import uuid # <<< إضافة لتوليد معرفات فريدة
...
from models.market_mood_analyzer import MarketMoodAnalyzer
...
class AdvisorManager:
    ...
    def __init__(self, storage, capital_manager, exchange=None, dl_client=None):
        ...
        self.instance_id = uuid.uuid4() # <<< إنشاء معرف فريد لهذه النسخة
        print(f"🕵️‍♂️ [DIAGNOSTIC] AdvisorManager instance created with ID: {self.instance_id}")
        ...
        self._creators = {
            ...
            'MarketMoodAnalyzer': lambda: MarketMoodAnalyzer(),
        ...
        print("✅ AdvisorManager (السكرتير الذكي) جاهز للعمل بنظام التحميل عند الطلب.")
            ...
            with self.__class__._lock:
                ...
                if advisor_name not in self.__class__._advisors:
                    if advisor_name in self._creators:
                        # <<< طباعة تشخيصية للمعرفة من أي نسخة يتم التحميل
                        print(f"🕵️‍♂️ [DIAGNOSTIC] Loading '{advisor_name}' via AdvisorManager instance ID: {self.instance_id}")
                        print(f"⏳ {advisor_name}: جاري التحميل لأول مرة...")
