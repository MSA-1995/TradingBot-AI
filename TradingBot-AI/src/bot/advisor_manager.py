...
class AdvisorManager:
    ...
    def __init__(self, storage, capital_manager, exchange=None, dl_client=None):
        ...
        self._creators = {
            ...
            # هذه النماذج تستخدم الذكاء الاصطناعي المدرب
...
class AIAdvisorWrapper:
    ...
    def get_confidence_adjustment(self, symbol, analysis):
        ...
        try:
            ...
            # تحويل النصيحة إلى تعديل رقمي
            if 'Strong-Bullish' in str(model_advice):
                return 15
            elif 'Bullish' in str(model_advice):
                return 8
            elif 'Strong-Bearish' in str(model_advice):
                return -15
            elif 'Bearish' in str(model_advice):
                return -8
            else:
                return 0
    ...
    def should_exit(self, symbol, position, current_price, analysis, mtf):
        ...
        if not self.has_trained_model:
            # Fallback: قرار بسيط بناءً على الربح
        ...
        try:
            ...
            return {'action': 'HOLD', 'reason': 'Error', 'profit': 0}
    ...
    def calculate_liquidity_score(self, symbol, analysis):
        ...
        try:
            ...
            if 'Strong-Bullish' in str(liquidity_advice):
                return 90
            elif 'Bullish' in str(liquidity_advice):
                return 70
            elif 'Strong-Bearish' in str(liquidity_advice):
                return 20
            elif 'Bearish' in str(liquidity_advice):
                return 40
            else:
                return 50
    ...
    def calculate_optimal_amount(self, symbol, confidence, base_amount=12, max_amount=700, whale_confidence=0):
        ...
        try:
            # استخدام النموذج المدرب لحساب المبلغ الأمثل
            # النموذج يتعلم من التاريخ ما هو المبلغ الأفضل
            if confidence >= 90:
                multiplier = 1.0
            elif confidence >= 80:
                multiplier = 0.8
