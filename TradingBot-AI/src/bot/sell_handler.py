...
def process_sell(result, exchange, ctx):
    ...
    if _FIB_AVAILABLE:
        try:
            ...
            at_sup, _ = fib.is_at_support(price, fib_data, tolerance=1.0, volume_ratio=vol_ratio, symbol=symbol)
            if at_sup and rsi < 30:
                print(f'☠️ SELL PAUSED [{symbol}]: Price at Fibonacci Support + RSI={rsi:.1f} Oversold')
                return False
        except Exception as e:
            print(f'⚠️ [{symbol}] Fibonacci sell filter error: {e}')
