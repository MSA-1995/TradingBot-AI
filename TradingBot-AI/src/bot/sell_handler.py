...
def process_sell(result, exchange, ctx):
    ...
    try:
        ...
        # Add to trade_data
        trade_data.update({
            'order_book_imbalance': safe_float(order_book_imbalance),
            'spread_volatility': safe_float(spread_volatility),
            'depth_at_1pct': safe_float(depth_at_1pct),
            'market_impact_score': safe_float(market_impact_score),
            'liquidity_trends': safe_float(liquidity_trends),
            'volatility_risk_score': safe_float(volatility_risk_score),
            'correlation_risk': safe_float(correlation_risk),
            'gap_risk_score': safe_float(gap_risk_score),
            'black_swan_probability': safe_float(black_swan_probability),
            'behavioral_risk': safe_float(behavioral_risk),
            'systemic_risk': safe_float(systemic_risk),
            'profit_optimization_score': safe_float(profit_optimization_score),
            'time_decay_signals': safe_float(time_decay_signals),
            'opportunity_cost_exits': safe_float(opportunity_cost_exits),
            'market_condition_exits': safe_float(market_condition_exits),
            'harmonic_patterns_score': safe_float(harmonic_patterns_score),
            'elliott_wave_signals': safe_float(elliott_wave_signals),
            ...
            'pattern_anomalies': safe_float(pattern_anomalies),
            'behavioral_anomalies': safe_float(behavioral_anomalies),
            'volume_anomalies': safe_float(volume_anomalies),
            'attention_mechanism_score': safe_float(attention_mechanism_score),
            'multi_scale_features': safe_float(multi_scale_features),
            'temporal_features': safe_float(temporal_features),
            'volume_trend_strength': safe_float(volume_trend_strength),
            'volume_volatility': safe_float(volume_volatility),
            'volume_momentum': safe_float(volume_momentum),
            'volume_seasonality': safe_float(volume_seasonality),
            'volume_correlation': safe_float(volume_correlation),
            'dynamic_consultant_weights': safe_float(dynamic_consultant_weights),
            'uncertainty_quantification': safe_float(uncertainty_quantification),
            'context_aware_score': safe_float(context_aware_score),
            'rsi': safe_float(analysis.get('rsi'), 50),
            'volume_ratio': safe_float(volume_ratio, 1),
            'sentiment_score': safe_float(real_sentiment_score, 0),
            'panic_score': safe_float(real_panic_score, 0)
        })
        # تحديث ذاكرة العملة
        try:
