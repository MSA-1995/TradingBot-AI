...
def process_sell(result, exchange, ctx):
    ...
    # إضافة أصوات البيع إذا كانت متوفرة
    voting_display = ""
    ...
    # إرسال الإشعار مع القيم الصحيحة
    send_sell_notification(
        symbol=symbol,
        amount=result['amount'],  # الكمية بالعملة (BTC, BNB, etc)
        price=result['price'],     # السعر الحالي
        value=sell_value,          # القيمة الإجمالية بالدولار
            ...
            if raw_candles and isinstance(raw_candles, list) and len(raw_candles) > 0:
                ...
                for candle in raw_candles[-7:]:
                    if isinstance(candle, dict):
                        ...
                        # فقط نحفظ الشمعة إذا كانت قيمها حقيقية
