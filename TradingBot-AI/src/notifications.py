...
def send_buy_notification(symbol, amount, price, value, confidence, tp_target=None, sl_target=None, buy_vote_percentage=None, buy_vote_count=None, total_consultants=None):
    ...
    fields = [
        ...
        {"name": "Quantity", "value": f"{amount:.2f}", "inline": True},
    ...
    if open_positions and len(open_positions) > 0:
        ...
        for i, (symbol, pos_data) in enumerate(sorted_positions):
            ...
            if is_opportunity:
                ...
                # صفقة مفتوحة - SL ديناميكي مطابق للطباعة
                ...
                whale_modifier = whales / 500
                base_threshold -= whale_modifier
                sentiment_modifier = sentiment / 200
                base_threshold += sentiment_modifier
                sl_threshold = max(atr * 0.5, min(atr * 3.0, base_threshold))

                # تعديل للخسائر كما في exit_strategy
                if profit_percent < 2:
                    sl_threshold *= 1.5
                actual_sl_threshold = 12.0  # الستوب الفعلي من wave protection
                stop_price = buy_price * (1 - actual_sl_threshold / 100)  # سعر البيع الفعلي
                stop_loss = f" | Stop Loss Threshold: {sl_threshold:.1f}% (Sell at: ${stop_price:.4f}) (ATR:{atr:.1f}%, Risk:{risk}, Whale Score:{whales:.0f}, Sentiment:{sentiment:.1f})" if profit_percent < -1.0 else ""
                entry = (
                    f"**{symbol}**\n"
                    f"Buy: ${buy_price:.4f} | Now: ${current_price:.4f}\n"
                    f"P/L: {profit_sign}{profit_percent:.2f}%{stop_loss}\n\n"
                )
