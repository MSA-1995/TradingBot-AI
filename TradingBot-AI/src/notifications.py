...
def send_positions_report(balance, invested, active_count, max_positions, open_positions=None):
    ...
    for i, (symbol, pos_data) in enumerate(sorted_positions):
        ...
        # ⚡ Real-time Stop Loss Trigger Check
        ...
        if remaining < 1.5 and drop_from_peak > 0:
            sl_warning = f" ⚡ **NEAR STOP** ({remaining:.1f}% left)"
        elif remaining < 0.5 and drop_from_peak > 0:
        ...
        entry = (
            ...
            f"P/L: {profit_sign}{profit_percent:.2f}% | Stop: {sl_threshold:.1f}% (${stop_price:.4f}){sl_warning}\n"
