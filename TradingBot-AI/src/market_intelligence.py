...
class TimeAnalyzer:
    ...
    def _get_recommendation(self):
        ...
        # نهاية الأسبوع
        if day >= 5:
            return {'action': 'CAUTION', 'message': 'نهاية الأسبوع', 'can_trade': False, 'reason': 'نهاية الأسبوع - سيولة منخفضة'}
        
        # جمعة متأخرة
        if day == 4 and hour >= 21:
            return {'action': 'AVOID', 'message': 'Friday Late', 'can_trade': False, 'reason': 'جمعه متأخرة'}
        
        # فترة هادئة
        if 21 <= hour < 24:
            return {'action': 'REDUCED', 'message': 'Quiet Hours', 'can_trade': True, 'size_multiplier': 0.5, 'reason': 'فترة هادئة'}
