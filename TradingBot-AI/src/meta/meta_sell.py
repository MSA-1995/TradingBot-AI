...
class SellMixin:
    ...
    def should_sell(self, symbol: str, position: dict,
        ...
        # MacroTrend (3)
        macro_p = (3 if 'BEAR'    in macro_status
                   else 1 if 'NEUTRAL' in macro_status
                   else 0)
        ...
        # Sell threshold by market
        required = (70 if 'BULL' in macro_status
                    else 50 if 'BEAR' in macro_status
                    else 60)
        ...
        if sell_points >= required:
            ...
            return {
                'action'         : 'SELL',
                'reason'         : (
                    f'👑 Peak:{sell_points:.0f}/{required}pts | '
                    f'Meta:{meta_conf:.0f}% | '
                    f'Core:{core_points:.0f}/40'),
                'profit'         : profit_pct,
                'sell_votes'     : core_votes,
    ...
    def _wave_protection(self, symbol, analysis, candles,
        ...
        # 🎯 Smart Peak Sell - voting system
        if profit_pct >= dyn_min:
            weights = {
                'candle_expert': 8, 'chart_cnn': 8, 'realtime_pa': 8,
                'trend_detector': 5, 'smart_money': 5,
                'multitimeframe': 4, 'volume_forecast': 2
            }
            total_weight = sum(weights.values())
            weighted_score = sum(
                (sell_votes.get(name, 0) / 100) * w
                for name, w in weights.items()
            )
            vote_confidence = (weighted_score / total_weight) * 100 if total_weight > 0 else 0

            if vote_confidence >= 60 and peak_score >= 60:
                gc.collect()
                return {
                    'action'             : 'SELL',
                    'reason'             : (f'🎯 Peak Vote: {vote_confidence:.0f}% | '
                                            f'Peak:{peak_score} | '
