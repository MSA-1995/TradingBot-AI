        ...
        try:
            ...
            if macro:
                ...
                ai['macro_prediction'] = {
                    ...
                    '1h': state.get('1h', {}).get('status', 'NEUTRAL'),
                    '4h': state.get('4h', {}).get('status', 'NEUTRAL'),
                    '1h_confidence': state.get('1h', {}).get('confidence', 50),
                    '4h_confidence': state.get('4h', {}).get('confidence', 50),
                    'direction': pd_dir,
                    'strength': pd_str,
                }
                ai['1h_bullish'] = 'BULL' in str(state.get('1h', {}).get('status', ''))
                ai['4h_bullish'] = 'BULL' in str(state.get('4h', {}).get('status', ''))
                ai['1h_bearish'] = 'BEAR' in str(state.get('1h', {}).get('status', ''))
                ai['4h_bearish'] = 'BEAR' in str(state.get('4h', {}).get('status', ''))
                dyn_min = max(
