...
def get_sentiment_data(symbol=None, analysis=None):
    ...
    # ========== 4. Whale Confidence من البيانات التقنية ==========
    if analysis:
        try:
            vol_ratio = float(analysis.get('volume_ratio', 1))
            btc_change = float(analysis.get('btc_change_1h', 0))
            ...
            # whale_confidence: حجم تداول عالي + سعر BTC مستقر + RSI طبيعي = ثقة عالية
            whale_conf = 0.0
            if vol_ratio > 2.0:   whale_conf += 3.0
            elif vol_ratio > 1.5: whale_conf += 1.5
            if abs(btc_change) < 1.0: whale_conf += 2.0
            if 40 <= rsi_val <= 65:   whale_conf += 2.0
            if vol_ratio > 1.5 and btc_change > 0: whale_conf += 1.0

            result['whale_confidence'] = round(min(whale_conf, 10.0), 2)
        except:
            result['whale_confidence'] = 0.0
