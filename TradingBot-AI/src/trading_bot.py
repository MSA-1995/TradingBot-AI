...
sell_cooldown = {}  # يجب أن يحتوي على datetime مع timezone.utc
    ...
    try:
        ...
        # ========== BUY LOGIC (Delegated to Meta) ==========
            ...
            with sell_cooldown_lock:
                if symbol in sell_cooldown and (datetime.now(timezone.utc) - sell_cooldown[symbol]).total_seconds() / 60 < COOLDOWN_MINUTES:
                    return {'symbol': symbol, 'action': 'COOLDOWN', 'minutes_left': COOLDOWN_MINUTES - ((datetime.now(timezone.utc) - sell_cooldown[symbol]).total_seconds() / 60)}
